...
const StudentPortal = (() => {
  ...
  const states = {satisfied: "Met", not_satisfied: "Needs attention", unresolved: "Not enough information yet", conflict: "Conflicting information", unsupported: "Not assessed by CRE"};
  const categories = {academic_record: "Academic results", course_completion_recognition: "Recognised courses", requirement_recognition: "Other recognition information", external_subject_achievement: "External subject results", prior_qualification: "Prior qualifications", registration_history: "Registration history", graduation_clearance: "Graduation information", qualification_award: "Award information"};
  ...
  function dashboard(view, config) {
    const items = canonical(view);
    const alerts = curriculum(view).filter(item => item.outcome !== "satisfied");
    alerts.sort((a, b) => ["conflict", "not_satisfied", "unresolved", "unsupported"].indexOf(a.outcome) - ["conflict", "not_satisfied", "unresolved", "unsupported"].indexOf(b.outcome));
    const decision = items.find(item => item.next_action === "SEEK_INSTITUTIONAL_DECISION");
    return heading("My curriculum", "What the information supplied means for you.") + summary(view) +
      `<section>${heading("What needs attention")}<ul class="portal-alerts">${alerts.slice(0, 3).map(item => `<li class="${esc(item.outcome)}"><strong>${esc(StudentLanguage.title(item, config))}</strong><p>${esc(StudentLanguage.meaning(item, config).label)}</p>${StudentLanguage.action(item, config) ? `<p>${esc(StudentLanguage.action(item, config))}</p>` : ""}</li>`).join("") || "<li>No curriculum issue is currently established.</li>"}</ul>${decision ? `<p class="human-boundary"><strong>${esc(StudentLanguage.title(decision, config))}:</strong> ${esc(StudentLanguage.action(decision, config))}. CRE cannot make that decision for you.</p>` : ""}<button data-view="curriculum">View all requirements</button></section>` +
      heading("Completion and next steps") + sequence(view, config) +
      `<div class="portal-links"><button data-view="evidence">Information supplied</button><button data-view="sources">Where the rules come from</button></div>`;
  }
  function renderCurriculum(view, config) {
    ...
    groups.push({title: "Other represented requirements", rows: curriculum(view).filter(item => !seen.has(item.identity))});
    Object.entries(kinds).forEach(([kind, title]) => groups.push({title, rows: all.filter(item => item.kind === kind)}));
    return heading("Your requirements", "Start with the result. Open Why? for the information behind it.") + groups.filter(group => group.rows.length).map(group => `<section class="portal-group">${heading(group.title)}${group.rows.map(item => studentConclusionCard(item, config)).join("")}</section>`).join("");
  }
  ...
  function sourceCard(source, links) {
    return `<article class="portal-source"><h3>${esc(source.title)}</h3>${links.map(link => `<details><summary>${esc(link.title)}</summary><p>${locator(link.locator)}</p><small>CRE links this location to the assessment. This does not mean the institution has formally confirmed CRE's interpretation.</small></details>`).join("")}</article>`;
  }
  ...
  function evidence(view, config, retrieval = [], duration = null) {
    ...
    return heading(`Information ${institution} supplied`, `${institution} remains the holder of your academic record. CRE uses information supplied for this session; not every item is used by every conclusion.`) +
  ...
  function help(config) {
    ...
    return heading("How this helps you") + `<div class="portal-help"><section><h3>What does CRE do?</h3><p>It explains what ${esc(institution)}'s represented rules and supplied information mean for your current curriculum questions.</p></section>
      ...
      <section><h3>Where do the rules come from?</h3><p>Sources shows the institutional locations linked to the represented rules. Information supplied explains what ${esc(institution)} sent about you. The two answer different questions.</p></section>
      <section><h3>About this demonstration</h3><p>These accounts contain no real student data. This is not production authentication or a security guarantee.</p></section></div>`;
  }
  return {counts, summary, dashboard, curriculum: renderCurriculum, evidence, sources, sourceCard, locator, help};
})();
