...
def test_regular_and_extended_route_rules_match_2026_handbook_structure():
    ...
    assert (regular.minimum_duration_years, regular.maximum_registration_years) == (
        3,
        4,
    )
    assert (extended.minimum_duration_years, extended.maximum_registration_years) == (
        4,
        5,
    )
    assert (
        regular.semester_course_equivalents
        == extended.semester_course_equivalents
        == 20
    )
    assert regular.senior_course_equivalents == extended.senior_course_equivalents == 10
    assert (
        regular.humanities_course_equivalents
        == extended.humanities_course_equivalents
        == 12
    )
...
def test_degree_title_rejects_two_ba_humanities_majors_for_bsocsc():
    ...
    identity = next(
        req for req in report.requirements if req.id == "degree_major_identity"
    )
...
def test_unknown_science_programme_is_rejected():
    response = client.post(
        ...
        json={
            "faculty": "uct_science",
            "programme_key": "generic_undergraduate",
            "student_id": "T5",
            "name": "Student",
            "programme": "BSc",
            "declared_majors": [],
...
def test_tdp_recognition_boundary_matches_fb5_2_and_keeps_the_major_pathway():
    ...
    recognised = {
        code
        for code, fact in catalogue.courses.items()
    ...
    assert (
        catalogue.courses["TDP1027F"].department
...
def test_extended_support_courses_add_credits_but_not_twenty_subject_course_equivalents():
    ...
    student = StudentRecord(
        "T6",
        "Student",
        "Bachelor of Social Science Extended",
        [],
    ...
    assert any(
        "not toward the 20 semester subject-course requirement" in warning
...
def test_music_recognition_limit_prevents_credit_inflation():
    ...
    student = StudentRecord(
        ...
        "Bachelor of Arts",
        [],
        [_passed(code) for code in codes],
        faculty_key="uct_humanities",
    ...
    assert any(
        "Music-course recognition limit applied" in warning
        for warning in report.warnings
    )
...
def test_first_year_non_humanities_course_is_only_recommended_inside_selected_external_major():
    ...
    no_major_suggestions = {
        course.code for course in _compute_eligible_courses(no_major, scoped)
    }
    ...
    suggestions = {
        course.code for course in _compute_eligible_courses(external_major, scoped)
    }
...
def test_standard_major_distinction_applies_first_attempt_and_mark_thresholds():
    ...
    repeated_politics = next(
        subject
        for subject in repeated_report.distinction.subjects
        if subject.major == "Politics & Governance"
    )
