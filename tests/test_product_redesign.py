...
def test_bootstrap_exposes_decision_model_and_lightweight_faculties():
    ...
    assert [lens["key"] for lens in data["decision_lenses"]] == [
        "position",
        "blockers",
        "next",
    ]
...
def test_student_and_admin_frontends_use_external_assets():
    ...
    assert "http://127.0.0.1:8000" not in Path("static/app.js").read_text(
        encoding="utf-8"
    )
...
def test_pdf_endpoint_returns_programme_scoped_report():
    parsed = StudentRecord(
        ...
        results=[
            CourseResult(
                "PHI1024F", "Introduction to Philosophy", 5, 18, 65, "2-", 2024
            )
        ],
    )
