...
def result(catalogue, code: str, mark: int | None = 75, year: int = 2026, grade: str | None = None):
    fact = catalogue.courses[code]
    return CourseResult(
        code, fact.name, fact.nqf_level, fact.nqf_credits,
        mark if fact.nqf_credits else None,
        grade or ("P" if fact.nqf_credits else "PA"), year,
    )
...
def complete_structured_record(programme: str, pathway: str = "", mark: int = 80):
    ...
    rows = [result(catalogue, code, mark, 2024 + index // 10) for index, code in enumerate(codes)]
    student = StudentRecord(
        "HLT001", "Health Student", programme_rules.name, [], rows,
        faculty_key="uct_health", programme_key=programme,
        pathway_key=pathway, years_registered=programme_rules.minimum_duration_years,
    )
...
def test_health_catalogue_contains_all_undergraduate_routes():
    ...
    assert {
        "mbchb", "mbchb_fundamentals", "bsc_medicine",
        "bsc_audiology", "bsc_speech_language_pathology",
        "bsc_occupational_therapy", "bsc_physiotherapy",
...
def test_complete_cosmetic_diploma_record_can_be_eligible_and_distinguished():
    catalogue, student = complete_structured_record("advanced_diploma_cosmetic_formulation")
...
def test_complete_audiology_record_still_requires_clinical_verification():
    ...
    assert any(
        row.id == "curriculum:clinical_professional_confirmation" and row.status == "unverified"
...
def test_fundamentals_route_requires_fundamentals_course():
    ...
    student.results = [row for row in student.results if row.code not in {"HSE1001F", "HSE1001S"}]
    report = compute_report(student, catalogue)
    row = next(item for item in report.requirements if item.id == "curriculum:fundamentals_course")
...
def test_bsc_medicine_credit_and_level_rules_can_be_satisfied():
    ...
    codes = [
        "HUB1006F", "IBS1007S", "PHY1025F", "PTY2000S", "HUB2017H",
        "MDN2001S", "FCE2000W", "SLL2002H", "PTY3009F", "HUB3006F",
        "HUB3007S", "IBS3020W", "AHS3078H",
    ]
    rows = [result(catalogue, code, 80) for code in codes]
    student = StudentRecord(
        "HLT002", "BSc Medicine Student", "BSc Medicine", [], rows,
        faculty_key="uct_health", programme_key="bsc_medicine",
        years_registered=1,
    )
    report = compute_report(student, catalogue)
...
def test_nmfc_route_never_claims_a_uct_medical_qualification_from_static_record():
    ...
    assert report.graduation_status == "requires_verification"
    assert any(row.id == "curriculum:nmfc_external_award" for row in report.requirements)
...
def test_first_year_health_failure_group_is_flagged():
    ...
    student = StudentRecord(
        "HLT003", "OT Student", "BSc Occupational Therapy", [], rows,
        faculty_key="uct_health", programme_key="bsc_occupational_therapy",
        years_registered=1,
    )
...
def test_failure_during_repeat_year_is_flagged():
    ...
    student = StudentRecord(
        "HLT004", "Physio Student", "BSc Physiotherapy", [], rows,
        faculty_key="uct_health", programme_key="bsc_physiotherapy",
        years_registered=2,
    )
    risk = compute_report(student, catalogue).exclusion_risk
    assert risk.at_risk
...
def test_mbchb_equivalent_block_code_satisfies_canonical_course_rule():
    ...
    report = compute_report(student, catalogue)
    row = next(item for item in report.requirements if item.id == "curriculum:mbchb_y4_aae4002w")
    assert row.complete
...
def test_disability_certificate_accepts_combined_course_layout():
    ...
    student = StudentRecord(
        "HLT005", "Certificate Student", "Higher Certificate in Disability Practice", [],
        [result(catalogue, code, 70) for code in codes],
        faculty_key="uct_health", programme_key="higher_certificate_disability_practice",
        years_registered=1,
    )
    report = compute_report(student, catalogue)
    curriculum = next(row for row in report.requirements if row.id == "curriculum:disability_curriculum")
