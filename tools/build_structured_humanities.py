    ...
    *,
    ...
    return {
        ...
        "level_credit_requirements": {
            str(k): v for k, v in (level_requirements or {}).items()
        },
...
def codes_from_rules(rules: list[dict[str, Any]]) -> set[str]:
    ...
    for r in rules:
        ...
        result.update(
            str(c).upper() for c in r.get("filters", {}).get("course_codes", [])
        )
...
programmes["higher_certificate_acet"] = programme(
    ...
    "HU052",
    min_credits=120,
    level_requirements={5: 120},
    min_years=1,
    rules=[
        all_courses(
            "hc_acet_core",
            "All Higher Certificate ACET modules",
            ["EDN1031FS", "EDN1030FS", "EDN1032FS"],
            page=21,
        )
    ],
...
programmes["advanced_certificate_foundation_phase"] = programme(
    ...
    "HU048",
    min_credits=120,
    level_requirements={6: 120},
    min_years=1,
    rules=[
        all_courses(
            "acfpt_core", "All Foundation Phase core modules", foundation, page=24
        ),
        manual(
            "acfpt_ict",
            "ICT competence condition",
...
programmes["advanced_certificate_intermediate_phase"] = programme(
    "Advanced Certificate in Intermediate Phase Teaching: English FAL and Mathematics",
    "HU045",
    min_credits=120,
    level_requirements={6: 120},
    min_years=1,
    rules=[
        all_courses(
            "acipt_core", "All Intermediate Phase core modules", intermediate, page=24
        ),
        manual(
            "acipt_ict",
            "ICT competence condition",
...
senior_paths = {
    "english_fal": (
        ...
        [f"EDN25{i:02d}W" for i in range(0, 7)],
        "EDN31",
    ),
    "mathematics": ("Mathematics", [f"EDN25{i:02d}W" for i in range(7, 14)], "EDN32"),
    "natural_sciences": (
        "Natural Sciences",
        [f"EDN25{i:02d}W" for i in range(14, 21)],
        "EDN33",
    ),
}
programmes["advanced_certificate_senior_phase"] = programme(
    ...
    pathways={
        k: path(
            f"{name} [{stream}]",
            [all_courses(f"acspt_{k}", f"All {name} modules", cs, page=25)],
            page=25,
        )
...
programmes["advanced_diploma_acet"] = programme(
    ...
    min_credits=120,
    level7=120,
    level_requirements={7: 120},
    min_years=2,
    rules=[
        all_courses(
            "ad_acet_core",
            "All Advanced Diploma ACET modules",
            ["EDN3700FS", "EDN3702FS", "EDN3701FS", "EDN3703FS"],
            page=27,
        )
    ],
    admission=[
        "Admission depends on an approved prior education qualification and at least two years of relevant experience."
    ],
    page=26,
)
...
programmes["advanced_diploma_school_leadership"] = programme(
    ...
    min_years=2,
    pathways={
        "first_semester_intake": path(
            "First-semester intake",
            [
                all_courses(
                    "slm_first_core",
                    "Modules in the first-semester intake pattern",
                    slm_first,
                    status="provisional",
                    page=28,
                ),
                manual(
                    "slm_first_continuation",
                    "Multi-year continuation allocation",
        ...
        "second_semester_intake": path(
            "Second-semester intake",
            [
                all_courses(
                    "slm_second_core",
                    "Modules in the second-semester intake pattern",
                    slm_second,
                    status="provisional",
                    page=29,
                ),
                manual(
                    "slm_second_continuation",
                    "Multi-year continuation allocation",
                    "Several W-coded workplace modules continue across registration years. The Faculty record must confirm their final credit allocation.",
                    status="unverified",
                    page=29,
    ...
    pathway_required=True,
    admission=[
        "Selection prioritises serving or aspirant school leaders and includes employment and experience conditions."
    ],
    page=28,
)
...
programmes["ba_screen_production"] = programme(
    "Bachelor of Arts specialising in Screen Production",
    "HB067",
    min_credits=360,
    level7=120,
    level_requirements={7: 120},
    ...
    rules=[
        all_courses(
            "screen_core", "Screen Production compulsory sequence", screen_core, page=40
        ),
        count(
            "screen_first_year",
            "Eight first-year semester courses",
            8,
        ...
        count(
            "screen_second_year",
            "Six second-year semester courses",
            6,
            filters={"nqf_levels": [6]},
            page=41,
        ),
        count(
            "screen_third_year",
            "Six third-year semester courses",
            6,
        ...
        manual(
            "screen_selection",
            "Selection into Screen Production",
    ...
    support=screen_core,
    admission=[
        "Selection into the specialisation is competitive and is not established by transcript data alone."
    ],
    page=40,
)
...
programmes["bsocsc_ppe"] = programme(
    ...
    rules=[
        ...
        all_courses(
            "ppe_year2_fixed", "Second-year compulsory courses", ppe_y2_fixed, page=43
        ),
        choose(
            "ppe_year2_politics", "One second-year Politics option", ppe_pol2, page=43
        ),
        choose(
            "ppe_year2_other", "One additional second-year option", ppe_other2, page=43
        ),
        all_courses(
            "ppe_eco3025",
            "Compulsory third-year Economics course",
            ["ECO3025S"],
            page=44,
        ),
        choose(
            "ppe_phi3", "At least one third-year Philosophy option", ppe_phi3, page=44
        ),
        choose(
            "ppe_pol3", "At least one third-year Politics option", ppe_pol3, page=44
        ),
        choose(
            "ppe_eco3",
            "At least one additional third-year Economics option",
            ppe_eco3,
            page=44,
        ),
        count(
            "ppe_third_count",
            "Six third-year PPE courses in total",
            6,
            codes=ppe_third_union,
            page=44,
        ),
    ],
    support=[*ppe_y1, *ppe_y2_fixed, *ppe_pol2, *ppe_other2, *ppe_third_union],
    thresholds=[
        {"year": 1, "minimum_passed_courses": 5},
        {"year": 2, "minimum_passed_courses": 10},
...
bsw_common = [
    rule_course(
        "bsw_psy1",
        "First introductory Psychology course",
        ["PSY1009F", "PSY1004F"],
        page=46,
    ),
    rule_course(
        "bsw_psy2",
        "Second introductory Psychology course",
        ["PSY1010S", "PSY1005S"],
        page=46,
    ),
    all_courses(
        "bsw_first_common",
        "Other first-year BSW courses",
        ["SOC1001F", "SOC1005S", "SWK1006S", "SWK1005S", "SWK1013F"],
        page=46,
    ),
    all_courses(
        "bsw_second_common",
        "Second-year Social Work sequence",
        ["SWK2001F", "SWK2060F", "SWK2065S", "SWK2070F", "SWK2075S"],
        page=46,
    ),
    all_courses(
        "bsw_third_common",
        "Third-year Social Work sequence",
        ["SWK3061F", "SWK3066S", "SWK3070F", "SWK3075S"],
        page=47,
    ),
    all_courses(
        "bsw_fourth",
        "Fourth-year Social Work sequence",
        ["SWK4015F", "SWK4016S", "SWK4030F", "SWK4031S", "SWK4032S", "SWK4033F"],
        page=47,
    ),
    manual(
        "bsw_professional_suitability",
        "Professional suitability and field-practice approval",
        "Progression includes field-practice, ethical suitability and professional conduct judgments that require departmental confirmation.",
        page=45,
    ),
]
bsw_paths = {
    "psychology": path(
        "Psychology professional-registration route",
        [
            choose(
                "bsw_psy_second",
                "Two second-year Psychology courses",
                ["PSY2013F", "PSY2014S", "PSY2015F"],
                2,
                page=46,
            ),
            all_courses(
                "bsw_psy_third",
                "Required third-year Psychology pair",
                ["PSY3005F", "PSY3011S"],
                page=47,
            ),
        ],
        support=["PSY2013F", "PSY2014S", "PSY2015F", "PSY3005F", "PSY3011S"],
        page=46,
    ),
    "sociology": path(
        "Sociology professional-registration route",
        [
            choose(
                "bsw_soc_second",
                "Two second-year Sociology courses",
                [
                    "PBL2800F",
                    "SOC2004S",
                    "SOC2015S",
            ...
            choose(
                "bsw_soc_third",
                "Two prescribed third-year Sociology courses",
                ["SOC3007F", "SOC3027F", "SOC3031S", "SOC3029S", "SOC3028S"],
                2,
                page=47,
            ),
        ],
...
programmes["bachelor_social_work"] = programme(
    ...
    min_credits=480,
    level7=120,
    level_requirements={7: 120, 8: 96},
    min_years=4,
    max_years=5,
    category="professional_bachelor",
    rules=bsw_common,
    pathways=bsw_paths,
    pathway_required=True,
    support=list(codes_from_rules(bsw_common)),
    thresholds=[
        {"year": 1, "minimum_passed_courses": 4},
        {"year": 2, "minimum_passed_courses": 8},
        {"year": 3, "minimum_passed_courses": 12, "minimum_senior_courses": 2},
        {"year": 4, "minimum_passed_courses": 16},
        {"year": 5, "minimum_passed_courses": 20},
    ],
    page=45,
)
...
fine_regular_rules = [
    all_courses(
        "fa_y1",
        "First-year Fine Art core",
        ["FIN1001W", "FIN1005W", "FIN1006F", "FIN1009S"],
        page=52,
    ),
    count(
        "fa_y1_elective",
        "One Humanities first-year elective",
        1,
        filters={
            "nqf_levels": [5],
            "exclude_prefixes": ["FIN"],
            "general_elective": True,
        },
        page=52,
    ),
    choose("fa_studio2", "Two second-year studio courses", fine_studio2, 2, page=52),
    all_courses(
        "fa_core2", "Second-year Fine Art core", ["FIN2026W", "FIN2028S"], page=52
    ),
    choose(
        "fa_discourse2",
        "One second-year Art History/Discourse course",
        ["FIN2027F", "FIN2029F"],
        page=52,
    ),
    count(
        "fa_support2",
        "Two recommended Humanities electives",
        2,
        filters={
            "nqf_levels": [5, 6],
            "exclude_prefixes": ["FIN"],
    ...
    choose(
        "fa_discourse3a",
        "One third-year Art History/Discourse first-semester course",
        ["FIN3026F", "FIN3028F"],
        page=53,
    ),
    choose(
        "fa_discourse3b",
        "One third-year Art History/Discourse second-semester course",
        ["FIN3027S", "FIN3029S"],
        page=53,
    ),
    all_courses(
        "fa_y4", "Fourth-year Fine Art core", ["FIN4015W", "FIN4012W"], page=53
    ),
    manual(
        "fa_studio_coherence",
        "Studio-stream continuity and selection",
        "Studio choice, progression and fourth-year placement must be confirmed by Michaelis because competitive selection and stream continuity are not inferable from a transcript.",
        page=52,
    ),
]
programmes["ba_fine_art_regular"] = programme(
    "Bachelor of Arts in Fine Art - regular programme",
    "HB008",
    min_credits=480,
    level7=120,
    level_requirements={7: 120, 8: 96},
    ...
    max_years=5,
    category="professional_bachelor",
    rules=fine_regular_rules,
    support=sorted(codes_from_rules(fine_regular_rules)),
    page=52,
)
...
fine_ext_rules = [
    all_courses(
        "fae_y1",
        "Extended first-year core",
        ["FIN1001W", "FIN1008W", "DOH1005F"],
        page=54,
    ),
    all_courses(
        "fae_y2_core",
        "Extended second-year core",
        ["FIN1006F", "FIN1009S", "FIN2026W"],
        page=54,
    ),
    ...
    count(
        "fae_hum_electives",
        "Two Humanities electives across years three and four",
        2,
        filters={
            "nqf_levels": [5],
            "exclude_prefixes": ["FIN"],
    ...
    all_courses(
        "fae_y4_core", "Extended fourth-year Fine Art core", ["FIN3030W"], page=55
    ),
    choose(
        "fae_discourse4a",
        "One third-year Art History/Discourse first-semester course",
    ...
    all_courses(
        "fae_y5", "Extended fifth-year Fine Art core", ["FIN4015W", "FIN4012W"], page=55
    ),
    manual(
        "fae_augmenting",
        "Required augmenting-course allocation",
        "The transcript must show the augmenting courses linked to the selected Humanities electives; the exact pair depends on those electives.",
        status="unverified",
        page=55,
    ),
    manual(
        "fae_studio_coherence",
        "Studio-stream continuity and selection",
        "Studio progression and final placement require Michaelis confirmation.",
        page=54,
    ),
]
programmes["ba_fine_art_extended"] = programme(
    "Bachelor of Arts in Fine Art - extended programme",
    "HB064",
    min_credits=480,
    level7=120,
    level_requirements={7: 120, 8: 96},
    ...
    max_years=5,
    category="professional_bachelor",
    rules=fine_ext_rules,
    support=sorted(codes_from_rules(fine_ext_rules)),
    availability="continuing_only",
    availability_note="No new intake in 2026; continuing students only.",
    page=54,
)
...
def music_pathways(
    ...
    *,
    ...
    credit_minima: dict[str, int] | None = None,
) -> dict[str, Any]:
    out: dict[str, Any] = {}
    for key, data in music[family].items():
        ...
        if 0 < len(core_codes) <= 10:
            rules.append(
                all_courses(
                    f"{family}_{key}_core",
                    f"Core sequence for {data['name']}",
                    core_codes,
                    status="provisional",
                    page=page,
                )
        ...
        rules.append(
            manual(
                f"{family}_{key}_combination",
                "Instrument, ensemble and theory combination",
        ...
        out[key] = path(
            ...
            support=all_codes,
            status="provisional",
            availability="continuing_only" if continuing else "open",
            availability_note=(
                "No new intake in 2026; continuing students only." if continuing else ""
            ),
            page=page,
        )
    return out
...
programmes["diploma_music_performance_regular"] = programme(
    "Diploma in Music Performance - regular programme",
    "HU021",
    min_credits=360,
    level_requirements={6: 120},
    min_years=3,
    ...
    pathway_required=True,
    admission=[
        "Admission and stream placement depend on audition and South African College of Music approval."
    ],
    page=62,
)
programmes["diploma_music_performance_extended"] = programme(
    "Diploma in Music Performance - extended programme",
    "HU035",
    min_credits=360,
    level_requirements={6: 120},
    min_years=4,
    max_years=4,
    category="diploma",
    pathways=music_pathways("dmp_extended_initial", 72, continuing=True),
    pathway_required=True,
    availability="continuing_only",
    availability_note="No new intake in 2026; continuing students only.",
    page=71,
...
programmes["advanced_diploma_opera"] = programme(
    ...
    min_credits=144,
    level7=132,
    level_requirements={7: 132},
    min_years=1,
    category="advanced_diploma",
    rules=[
        all_courses(
            "ad_opera_core",
            "All Advanced Diploma in Opera modules",
            ["MUZ3377H", "MUZ3702H", "MUZ3704W", "SLL1094H"],
            page=76,
        )
    ],
    availability="not_offered",
    availability_note="Not offered in 2026.",
    page=76,
)
programmes["advanced_diploma_music"] = programme(
    ...
    rules=[
        ...
        choose(
            "ad_music_option",
            "One advanced repertoire/transcription option",
            ["MUZ3707H", "MUZ3708H", "MUZ3709H"],
            page=77,
        ),
    ],
    availability="not_offered",
...
programmes["bachelor_music_regular"] = programme(
    "Bachelor of Music - regular programme",
    "HB010",
    min_credits=480,
    level7=120,
    level_requirements={7: 120, 8: 96},
    ...
    max_years=5,
    category="professional_bachelor",
    pathways=music_pathways("bmus_regular", 78),
    pathway_required=True,
    admission=[
        "Admission and later stream placement can depend on audition, theory examination, performance standard and programme capacity."
    ],
    page=77,
)
programmes["bachelor_music_extended"] = programme(
    "Bachelor of Music - extended programme",
    "HB034",
    min_credits=480,
    level7=120,
    level_requirements={7: 120, 8: 96},
    ...
    max_years=5,
    category="professional_bachelor",
    pathways=music_pathways("bmus_extended_initial", 99, continuing=True),
    pathway_required=True,
    availability="continuing_only",
    availability_note="No new intake in 2026; continuing students only.",
    page=99,
...
dip_t_common = [
    all_courses(
        "dtp_y1",
        "First-year Diploma in Theatre and Performance core",
        ["TDP1046W", "TDP1017H", "TDP1027F", "TDP1045S", "TDP1029F", "DOH1005F"],
        page=112,
    ),
    all_courses(
        "dtp_y2",
        "Second-year Diploma in Theatre and Performance core",
        ["TDP2010F", "TDP2011S", "TDP2042F", "TDP2013S", "TDP1018H", "TDP2040W"],
        page=112,
    ),
    all_courses(
        "dtp_professional", "Third-year professional practice", ["TDP3052W"], page=113
    ),
    minimum_mark(
        "dtp_studiowork_mark",
        "First-year Studiowork progression mark",
        ["TDP1046W"],
        60,
        page=113,
    ),
    manual(
        "dtp_concentration_selection",
        "Concentration selection",
        "Concentration is allocated after audition and is not guaranteed by transcript choice alone.",
        page=110,
    ),
]
dip_t_paths = {
    "acting": path(
        ...
        [
            all_courses(
                "dtp_acting", "Acting third-year Studiowork", ["TDP3043W"], page=113
            )
        ],
        support=["TDP3043W"],
        page=110,
    ),
    "dance_performance": path(
        "Dance Performance",
        [
            all_courses(
                "dtp_dance", "Dance third-year Studiowork", ["TDP3047W"], page=113
            )
        ],
        support=["TDP3047W"],
        page=110,
    ),
    "performance_making": path(
        "Performance Making",
        [
            all_courses(
                "dtp_making",
                "Performance Making third-year Studiowork",
                ["TDP3041W"],
                page=113,
            )
        ],
    ...
    "applied_pedagogy": path(
        "Applied Performance / Pedagogy",
        [
            all_courses(
                "dtp_applied",
                "Applied/Pedagogy third-year Studiowork",
                ["TDP3050W"],
                page=113,
            )
        ],
    ...
    "scenography": path(
        ...
        [
            all_courses(
                "dtp_scenography",
                "Scenography third-year Studiowork",
                ["TDP3051W"],
                page=113,
            )
        ],
...
programmes["diploma_theatre_performance"] = programme(
    ...
    category="diploma",
    rules=dip_t_common,
    pathways=dip_t_paths,
    pathway_required=True,
    support=sorted(codes_from_rules(dip_t_common)),
    admission=["Admission is by audition and restricted selection."],
    page=109,
)
programmes["advanced_diploma_theatre"] = programme(
    ...
    rules=[
        all_courses(
            "ad_theatre_core",
            "All Advanced Diploma in Theatre modules",
            ["TDP3010F", "TDP3018S", "TDP3902W"],
            page=114,
        )
    ],
    availability="not_offered",
...
batp_common = [
    all_courses(
        "batp_y1_core",
        "First-year Theatre and Performance core",
        ["TDP1046W", "TDP1017H", "TDP1027F", "TDP1045S"],
        page=116,
    ),
    count(
        "batp_y1_electives",
        "Two non-TDP first-year courses",
        2,
        filters={
            "nqf_levels": [5],
            "exclude_prefixes": ["TDP"],
            "general_elective": True,
        },
        page=116,
    ),
    all_courses(
        "batp_y2_core",
        "Second-year Theatre and Performance core",
        ["TDP2010F", "TDP2011S", "TDP1018H", "TDP2040W"],
        page=117,
    ),
    count(
        "batp_y2_electives",
        "Two non-TDP second-year courses",
        2,
        filters={
            "nqf_levels": [6],
            "exclude_prefixes": ["TDP"],
    ...
    all_courses(
        "batp_y3_core",
        "Third-year professional core",
        ["TDP3010F", "TDP3018S", "TDP3052W"],
        page=118,
    ),
    count(
        "batp_y3_electives",
        "Two non-TDP third-year courses",
        2,
        filters={
            "nqf_levels": [7],
            "exclude_prefixes": ["TDP"],
    ...
    all_courses(
        "batp_y4_research", "Fourth-year research component", ["TDP4000H"], page=119
    ),
    minimum_mark(
        "batp_studiowork_mark",
        "First-year Studiowork progression mark",
        ["TDP1046W"],
        60,
        page=119,
    ...
    manual(
        "batp_progression",
        "Performance progression and selection",
        "Progression beyond the represented mark and course thresholds depends on successful auditions, prerequisites, professional conduct and concentration placement.",
        page=115,
    ),
]
batp_pairs = {
...
batp_paths = {
    key: path(
        ...
        [
            all_courses(
                f"batp_{key}",
                f"Matched third- and fourth-year {name} Studiowork",
                [third, fourth],
                page=119,
            )
        ],
        support=[third, fourth],
        page=115,
    )
...
programmes["ba_theatre_performance"] = programme(
    ...
    pathways=batp_paths,
    pathway_required=True,
    support=sorted(codes_from_rules(batp_common)),
    admission=["Admission is restricted and requires audition/selection."],
    page=115,
)
...
if missing:
    raise SystemExit(
        "Missing course facts for structured routes: " + ", ".join(missing)
    )
