...
PREFIX_DEPARTMENT = {
    "CML": "Commercial Law", "PVL": "Private Law", "PBL": "Public Law",
    "DOL": "Dean of Law", "ELL": "English Literary Studies",
    "SLL": "Languages and Literatures", "MAM": "Mathematics and Applied Mathematics",
    "PTY": "Pathology",
}
...
def offering(code: str) -> list[str]:
    ...
    return {
        ...
        "W": ["Whole year"], "H": ["Half course over whole year"],
        "X": ["Non-standard period"], "Z": ["Other/non-standard period"],
    }.get(suffix, [])


def fact(code: str, name: str, credits: int, level: int, page: int,
         *, offered: bool = True, prerequisites: list[str] | None = None,
         co_requisites: list[str] | None = None, note: str = "") -> dict[str, Any]:
    return {
        "code": code, "name": name, "credits": credits, "nqf_level": level,
        "prerequisites": prerequisites or [], "prerequisites_verified": True,
        "co_requisites": co_requisites or [],
        "offered": offering(code) if offered else [], "offering_verified": True,
        "department": PREFIX_DEPARTMENT.get(prefix(code), prefix(code)),
        "description": "", "verification_status": "verified",
        "source": {"document": SOURCE, "page": page, "section": "LLB curriculum/course outline"},
        ...
        "recognition_note": note or "Counts only inside a selected LLB route or its published elective/research component.",
    }


def course_rule(code: str, label: str | None = None) -> dict[str, Any]:
    return {"type": "course", "id": f"course_{code.lower()}", "label": label or f"Pass {code}", "course_codes": [code]}


def choose(rule_id: str, label: str, codes: list[str], required: int = 1) -> dict[str, Any]:
...
def approved_pool(rule_id: str, label: str, required: int, *, filters: dict[str, Any], exclude: list[str] | None = None) -> dict[str, Any]:
    return {
        "type": "approved_credit_pool", "id": rule_id, "label": label,
        "required": required, "course_codes": [], "transcript_course_codes": [],
        "exclude_course_codes": exclude or [], "transcript_filters": filters,
...
LECTURE_ELECTIVES = {
    "DOL4500F": ("Legal Practice", True), "DOL4500S": ("Legal Practice", True),
...
ALL_FINAL_ELECTIVES = sorted(set(LECTURE_ELECTIVES) | set(RESEARCH_ELECTIVES) | {"DOL3000X"})
SEMINAR_RESEARCH = sorted(set(RESEARCH_ELECTIVES) - set(INDEPENDENT_RESEARCH))

LANGUAGE_PAIRS = [
    ["SLL1022F", "SLL1023S"], ["SLL1050F", "SLL1051S"],
    ["SLL1052F", "SLL1053S"], ["SLL1058F", "SLL1059S"],
    ["SLL1060F", "SLL1061S"], ["SLL1062F", "SLL1063S"],
    ["SLL1064F", "SLL1065S"], ["SLL1073F", "SLL1074S"],
    ["SLL1075F", "SLL1076S"], ["SLL1082F", "SLL1083S"],
    ["SLL1101F", "SLL1102S"], ["SLL1121F", "SLL1122S"],
    ["SLL1131F", "SLL1132S"],
]
...
def final_component_rules() -> list[dict[str, Any]]:
    return [
        credit_pool("final_elective_credits", "Final Level research/elective component", ALL_FINAL_ELECTIVES, 36),
        any_of("research_requirement", "Seminar/research-paper requirement", [
            choose("seminar_research_elective", "Complete a seminar and research-paper elective", SEMINAR_RESEARCH, 1),
            choose("independent_research_paper", "Complete an 8,000-word independent research paper", INDEPENDENT_RESEARCH, 1),
            course_rule("DOL3000X", "Approved Moot Competition substitution (Moot Caput option)"),
        ]),
        {"type": "maximum_credit_pool", "id": "maximum_final_electives", "label": "Maximum Final Level elective credits", "course_codes": ALL_FINAL_ELECTIVES, "maximum": 54, "blocking": True},
    ]


def language_rule() -> dict[str, Any]:
    known = [
        {"type": "all_courses", "id": f"language_pair_{i}", "label": "Complete a two-semester language sequence", "course_codes": pair}
        for i, pair in enumerate(LANGUAGE_PAIRS, 1)
    ]
    known.append(approved_pool(
        "unlisted_language_sequence", "Other approved whole/two-semester language sequence", 30,
        filters={"prefixes": ["SLL"], "nqf_levels": [5]},
    ))
    return any_of("language_requirement", "Language requirement", known)


def other_faculty_rule(codes: list[str], required: int, rule_id: str, label: str) -> dict[str, Any]:
    return any_of(rule_id, label, [
        credit_pool(rule_id + "_known", label + " (catalogued options)", codes, required),
        approved_pool(rule_id + "_unlisted", label + " (other transcript course)", required,
                      filters={"nqf_levels": [5], "exclude_prefixes": ["CML", "PVL", "PBL", "DOL"]}),
    ])
...
def award_rules(min_years: int) -> list[dict[str, Any]]:
    ...
    return [
        {
            "name": "Magna cum laude", "complete_within_years": min_years,
            "curriculum_rules": [
                {"type": "no_failures", "id": "magna_no_failures", "label": "No failed course attempts", "condonable": True},
                {"type": "weighted_average", "id": "magna_average", "label": "Average across UCT law courses", "filters": law_filter, "minimum_average": 75},
                {"type": "passed_mark_equivalents", "id": "magna_firsts", "label": "First-class law-course equivalents", "filters": law_filter, "minimum_mark": 75, "required": 9, "equivalent_credit_unit": 36},
            ],
        },
        {
            "name": "Cum laude", "complete_within_years": min_years,
            "curriculum_rules": [
                {"type": "no_failures", "id": "cum_no_failures", "label": "No failed course attempts", "condonable": True},
                {"type": "weighted_average", "id": "cum_average", "label": "Average across UCT law courses", "filters": law_filter, "minimum_average": 70},
                {"type": "passed_mark_equivalents", "id": "cum_firsts", "label": "First-class law-course equivalents", "filters": law_filter, "minimum_mark": 75, "required": 6, "equivalent_credit_unit": 36},
            ],
        },
    ]
...
def progression(max_years: int, failed_threshold: int = 4) -> list[dict[str, Any]]:
    return [
        {"type": "failed_course_equivalents", "label": "Annual failed-course-equivalent threshold", "threshold": failed_threshold},
        {"type": "maximum_years", "label": "Prescribed time plus one year", "maximum": max_years},
    ]


def programme(name: str, code: str, total: int, years: int, required: list[str], page: int,
              *, rules: list[dict[str, Any]] | None = None, availability: str = "open",
              availability_note: str = "", max_years: int | None = None,
              ...
              admission_notes: list[str] | None = None, failed_threshold: int = 4) -> dict[str, Any]:
    return {
        "name": name, "minimum_nqf_credits": total, "minimum_nqf_level_7_credits": 0,
        ...
        "qualification_codes": [code], "scope_verified": True, "route_type": "structured",
        "degree_category": "Law", "minimum_duration_years": years,
        "maximum_registration_years": max_years or years + 1,
        "programme_type": "structured", "curriculum_rules": rules or [],
        "pathways": pathways or {}, "pathway_required": pathway_required,
        "default_pathway_key": "", "availability": availability,
        ...
        "source": {"document": SOURCE, "page": page, "section": "Admission and Curriculum Rules"},
    }
...
def main() -> None:
    ...
    courses["DOL3000X"] = fact("DOL3000X", "Moot Competition", 9, 7, 57,
    ...
    courses["MAM1013F"] = fact("MAM1013F", "Law That Counts: Quantitative Literacy for Law", 18, 5, 55)
    ...
    for code, (name, credits, level) in old.items():
        courses[code] = fact(code, name, credits, level, 52, offered=False,
    ...
    required_hum = {code for pair in LANGUAGE_PAIRS for code in pair} | {"ELL1013F", "ELL1016S"}
    for code, row in hum_by_code.items():
        ...
        copied["recognition_note"] = "Potential non-law component for the LLB; it counts only when it satisfies the selected route's published non-law category."
        courses.setdefault(code, copied)

    english_options = [code for code in ["ELL1013F", "ELL1016S", "SLL1002S"] if code in courses]
    reserved_level5 = set(english_options) | {"MAM1013F"} | {code for pair in LANGUAGE_PAIRS for code in pair}
    first_year_nonlaw = sorted(
        code for code, row in courses.items()
        if row.get("nqf_level") == 5 and prefix(code) not in {"CML", "PVL", "PBL", "DOL"}
        and code not in reserved_level5
    )
    second_year_nonlaw = sorted(
        code for code, row in courses.items()
        if row.get("nqf_level") == 6 and prefix(code) not in {"CML", "PVL", "PBL", "DOL"}
    )
    ...
    four_common = [
        choose("english_or_word_power", "English course or Word Power", english_options, 1),
        language_rule(),
        {
            "type": "same_department_credit_pool", "id": "second_year_nonlaw_discipline",
            "label": "Two 2000-level semester courses in one non-law discipline",
            "course_codes": second_year_nonlaw, "required": 40,
            "filters": {"nqf_levels": [6], "exclude_prefixes": ["CML", "PVL", "PBL", "DOL"]},
            "transcript_filters": {"nqf_levels": [6], "year_levels": [2], "exclude_prefixes": ["CML", "PVL", "PBL", "DOL"]},
    ...
    pathways = {
        "numeracy_course_required": {
            ...
            "curriculum_rules": [other_faculty_rule(first_year_nonlaw, 30, "first_year_other_faculty_30", "First-year courses in another faculty")],
            "verification_status": "verified", "availability": "open",
        ...
        "numeracy_test_passed": {
            ...
            "required_courses": [],
            "curriculum_rules": [other_faculty_rule(first_year_nonlaw, 45, "first_year_other_faculty_45", "First-year courses in another faculty")],
            "verification_status": "verified", "availability": "open",
    ...
    five_replacement_rules = [
        any_of("five_foundations", "Foundations of South African Law", [course_rule("PVL1006W"), course_rule("PVL1003W")]),
        any_of("five_persons", "Law of Persons and Family", [course_rule("PVL1009S"), course_rule("PVL1008H")]),
        any_of("five_private_system", "South African Private Law: System and Context", [course_rule("PVL1007F"), course_rule("PVL1004F")]),
        any_of("five_constitutional", "Constitutional Law", [course_rule("PBL2002W"), course_rule("PBL2000W")]),
    ]
    five_required = [
        code for code in law_core
        if code not in {"PVL1003W", "PVL1004F", "PVL1008H", "PBL2000W"}
    ] + ["SLL1002S", "MAM1013F"]

    programmes = {
        "llb_three_year_graduate": programme(
            "LLB - Three-year graduate stream", "LP001", 504, 3, law_core, 41,
            rules=common_final, max_years=4, prereq_overrides=grad_pr, coreq_overrides=grad_co,
            admission_notes=["Admission requires an appropriate completed bachelor's degree and Faculty admission under the Undergraduate Prospectus."],
        ),
        "llb_two_year_combined": programme(
            "LLB - Two-year graduate stream after Law and Humanities/Commerce", "LP001", 504, 2, law_core, 50,
            rules=common_final, max_years=3, prereq_overrides=ug_pr, coreq_overrides=ug_co,
            admission_notes=[
        ...
        "llb_four_year_undergraduate": programme(
            "LLB - Four-year undergraduate stream", "LB002", 637, 4, law_core, 43,
            rules=four_common, max_years=5, pathways=pathways, pathway_required=True,
            prereq_overrides=ug_pr, coreq_overrides=ug_co,
            admission_notes=["Select the numeracy pathway using the official test outcome; the test may not be attempted a second time."],
        ),
        "llb_five_year_continuing": programme(
            "LLB - Five-year undergraduate curriculum (continuing students)", "LB003", 637, 5, five_required, 52,
            rules=[
                *five_replacement_rules,
                other_faculty_rule(first_year_nonlaw, 30, "five_first_year_nonlaw", "First/second-year non-law option credits"),
                language_rule(),
                {
                    "type": "same_department_credit_pool", "id": "five_second_year_nonlaw_discipline",
                    "label": "Two 2000-level semester courses in one non-law discipline",
                    "course_codes": second_year_nonlaw, "required": 40,
                    "filters": {"nqf_levels": [6], "exclude_prefixes": ["CML", "PVL", "PBL", "DOL"]},
                    "transcript_filters": {"nqf_levels": [6], "year_levels": [2], "exclude_prefixes": ["CML", "PVL", "PBL", "DOL"]},
            ...
            max_years=6, prereq_overrides=ug_pr, coreq_overrides=ug_co,
            admission_notes=["Legacy courses no longer offered are recognised through their published replacement-course alternatives."],
            failed_threshold=3,
        ),
    }

    requirements = {
        "source": f"{SOURCE}; {GENERAL}", "catalogue_version": "2026.1-law",
        "programmes": programmes, "majors": {}, "forbidden_major_combinations": [],
        "cross_credit_exclusions": [],
    }
    (OUT / "courses.json").write_text(json.dumps(sorted(courses.values(), key=lambda row: row["code"]), indent=2) + "\n")
    (OUT / "degree_requirements.json").write_text(json.dumps(requirements, indent=2) + "\n")
    print(f"Wrote {len(courses)} Law-scoped course facts and {len(programmes)} LLB routes.")
