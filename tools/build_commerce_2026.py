...
PREFIX_DEPARTMENT = {
    ...
    "BUS": "School of Management Studies",
    "DOC": "Education Development Unit",
    ...
    "END": "Engineering and the Built Environment",
...
def offered_from_code(code: str) -> list[str]:
    ...
    return {
        ...
        "U": ["Summer term"],
        "L": ["Winter term"],
        "Q": ["Online first semester"],
        "R": ["Online second semester"],
    }.get(suffix, [])
...
def course_rule(
    codes: Iterable[str], label: str, rule_id: str, **extra: Any
) -> dict[str, Any]:
...
def all_of(
    children: list[dict[str, Any]], label: str, rule_id: str, **extra: Any
) -> dict[str, Any]:
    row: dict[str, Any] = {
        "type": "all_of",
        "id": rule_id,
...
def choose(
    codes: Iterable[str], required: int, label: str, rule_id: str, **extra: Any
) -> dict[str, Any]:
    row: dict[str, Any] = {
        "type": "choose_n",
        "id": rule_id,
...
def parse_course_descriptions(text: str) -> dict[str, dict[str, Any]]:
    ...
    for index, match in enumerate(matches):
        ...
        credit = re.search(
            r"(\d+)\s+NQF credits? at NQF level\s+(\d+)", segment[:2200], re.I
        )
        if not credit:
            continue
        ...
        if entry_text and re.fullmatch(
            r"None\.?|Admission to (?:the )?degree\.?.*", entry_text, re.I
        ):
            ...
            unsafe = re.search(
                r"\bor\b|permission|average|minimum|at least|concurrent|equivalent|registration for|%|admission",
                entry_text,
                re.I,
            )
            if found and not unsafe:
                prereqs = sorted(set(found))
                prereq_verified = True
        elif level == 5:
            prereq_verified = True
        ...
        if coreq_match:
            ...
            if not re.fullmatch(r"None\.?", coreq_text, re.I):
                for token in re.findall(
                    r"\b[A-Z]{3}\d{4}[A-Z](?:/[A-Z])?\b", coreq_text
                ):
        ...
        description = (
            re.sub(r"\s+", " ", outline.group(1)).strip()[:900] if outline else ""
        )
        not_offered = bool(
            re.search(r"not offered in 2026", title + " " + segment[:700], re.I)
        )
        for code in expand_code(raw_code):
            candidate = {
                ...
                "co_requisites": sorted(set(co_reqs)),
                ...
                "source": source(page, "Course outline"),
                "general_elective": False,
                "counts_towards_general_degree": True,
                "counts_as_humanities": False,
                "counts_as_science": False,
                "counts_towards_course_equivalents": True,
...
SPECIALISATIONS_BBUS = [
    ("BUS01", "Actuarial Science"),
    ("BUS09", "Actuarial Science specialising in Quantitative Finance"),
    ("STA13", "Statistics and Data Science"),
    ("FTX05", "Finance"),
    ("FTX04", "Finance with Accounting"),
    ("CSC05", "Computer Science"),
    ("INF01", "Information Systems"),
    ("ECO01", "Economics"),
    ("ECO03", "Economics with Law"),
    ("BUS07", "Marketing"),
...
SPECIALISATIONS_BCOM = [
    ...
    ("INF01", "Information Systems"),
    ("INF06", "Information Systems and Computer Science"),
    ("INF11", "Information Systems and Finance"),
    ("PHI03", "Philosophy, Politics and Economics"),
    ("ECO02", "Economics and Finance"),
    ("ECO04", "Economics and Statistics"),
    ("ECO03", "Economics with Law"),
    ("BUS06", "Management Studies"),
]
...
def route_specs() -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = [
        {
            "code": "CU020BUS01",
            "name": "Advanced Diploma in Actuarial Science",
            "family": "advanced",
            "mode": "standard",
            "page": 20,
        },
        {
            "code": "CU021GSB48",
            "qualification_codes": ["CU021GSB48", "CU022GSB48", "CU023GSB48"],
            "name": "Advanced Diploma in Management Development",
            "family": "advanced",
            "mode": "standard",
            "page": 21,
        },
            ...
            rows.append(
                {
                    ...
                    "name": f"Bachelor of Business Science in {title}",
                    "family": "bbussci",
                    "mode": mode,
...
ROW_RE = re.compile(
    r"^\s*([A-Z]{3}\d{4}[A-Z](?:/[A-Z])*)\s+(.+?)\s+(\d+)(?:-\d+)?\+?\s+([5-8])\s*$"
)
...
def positions_for_specs(
    text: str, specs: list[dict[str, Any]]
    ...
    for spec in specs:
        code = spec["code"]
        candidates = [
            m.start() for m in re.finditer(r"\[" + re.escape(code) + r"\]", text)
        ]
        if not candidates:
            ...
            candidates = [
                m.start() for m in re.finditer(r"\b" + re.escape(code) + r"\b", text)
            ]
        ...
        for candidate in candidates:
            ...
            has_curriculum = bool(
                re.search(
                    r"First Year (?:Core )?Modules|Prescribed curriculum", after, re.I
                )
            )
            heading_context = bool(
                re.search(r"Bachelor|Advanced Diploma", before + after[:240], re.I)
            )
        ...
        if not ranked:
            raise RuntimeError(
                f"Could not locate programme curriculum heading for {code}"
            )
...
def row_rule(row: dict[str, Any], route_code: str, index: int) -> dict[str, Any]:
    ...
    for code in codes:
        ...
        if alternatives:
            branches = [
                course_rule(
                    [code], f"Complete {code}", f"{route_code}_{index}_{code.lower()}"
                )
            ]
            for alt_index, alt_codes in enumerate(alternatives):
                if len(alt_codes) == 1:
                    branches.append(
                        course_rule(
                            alt_codes,
                            f"Complete recognised substitute {alt_codes[0]}",
                            f"{route_code}_{index}_{code.lower()}_sub{alt_index}",
                        )
                    )
                else:
                    branches.append(
                        all_courses(
                            alt_codes,
                            "Complete recognised substitute sequence",
                            f"{route_code}_{index}_{code.lower()}_sub{alt_index}",
                        )
                    )
            children.append(
                any_of(
                    branches,
                    f"{row['name']} or recognised equivalent",
                    f"{route_code}_{index}_{code.lower()}_equiv",
                )
            )
        else:
            children.append(
                course_rule(
                    [code],
                    f"Complete {row['name']} ({code})",
                    f"{route_code}_{index}_{code.lower()}",
                )
            )
    if len(children) == 1:
        return children[0]
...
def parse_block(
    block_name: str, block_lines: list[tuple[int, str]], route_code: str
    ...
    if block_name != "Programme":
        ...
        totals = [
            ...
            if re.match(
                r"^Total credits(?:\s+(?:per year|for (?:the )?year|for the degree|of NQF level))?",
                line,
                re.I,
            )
        ]
    ...
    for index, clean in enumerate(clean_lines):
        if re.search(
            r"(?:Mathematical Statistics|Applied Statistics) Option", clean, re.I
        ):
    ...
    if len(option_markers) >= 2:
        ...
        for marker_index, (start, label) in enumerate(option_markers):
                ...
                option_markers[marker_index + 1][0]
            ...
            if marker_index + 1 == len(option_markers):
                ...
                stops = [
                    ...
                    if re.search(r"^(Plus|Total)", clean_lines[i], re.I)
                ]
                if stops:
            ...
            if group:
                ...
                branch_rules = [
                    row_rule(row, route_code, row["line_index"]) for row in group
                ]
                branches.append(
                    all_of(
                        branch_rules,
                        label.strip(" :."),
                        f"{route_code}_{block_name}_option_{marker_index}",
                    )
                )
        if len(branches) >= 2:
            rules.append(
                any_of(
                    branches,
                    f"{block_name}: complete one statistics option",
                    f"{route_code}_{block_name}_statistics_option",
                )
            )
    ...
    for index, clean in enumerate(clean_lines):
        ...
        if re.search(r"\bfrom\b|students are required to take", clean, re.I):
            explicit_pool = re.search(
                r"(?:Plus,?|Take|.*students are required to take)\s+(\d+|one|two|three|four)\s+(?:other\s+)?(?:NQF[^:]*\s+)?(?:courses?|options?|electives?)\s*(?:from(?: the list below)?)?\s*(?::|in)?",
                clean,
                re.I,
            )
        ...
        stop_candidates = [
            i
            for i in range(index + 1, len(clean_lines))
            if re.search(
                r"^(Total credits|First Year|Second Year|Third Year|Fourth Year|Fifth Year|Plus \d+|Plus one|Plus two|Plus three)",
                clean_lines[i],
                re.I,
            )
        ]
        ...
        pool_rows = [
            row
            for row in rows
            if index < row["line_index"] < end and row["line_index"] not in consumed
        ]
        generic_options = [
            clean_lines[i]
            for i in range(index + 1, end)
            if re.match(
                r"^(Any |OR NQF|NQF level|[123]000 level)", clean_lines[i], re.I
            )
        ]
        ...
        if generic_options:
            ...
            rules.append(
                {
                    ...
                    "id": f"{route_code}_{block_name}_mixed_pool_{index}",
            ...
            rules.append(
                choose(
                    [code for row in pool_rows for code in row["codes"]],
                    required,
    ...
    while i < len(available):
        ...
        while j + 1 < len(available):
            between = clean_lines[
                available[j]["line_index"] + 1 : available[j + 1]["line_index"]
            ]
            if any(
                re.fullmatch(r"OR[ .]*", item, re.I)
                or re.search(r"\.\.OR\.", item, re.I)
                for item in between
            ):
                group.append(available[j + 1])
                j += 1
            else:
                break
        if len(group) > 1:
            ...
            rules.append(
                any_of(
                    branches,
                    f"{block_name}: complete one published alternative",
                    f"{route_code}_{block_name}_or_{group[0]['line_index']}",
                )
            )
            i = j + 1
        else:
        ...
        if mixed_and:
            ...
            branch1 = all_of(
                [
                    course_rule(
                        [code.upper()],
                        f"Complete {code.upper()}",
                        f"{route_code}_{block_name}_mixed_{index}_course",
                    ),
                    {
                        "type": "approved_credit_pool",
                        "id": f"{route_code}_{block_name}_mixed_{index}_a",
                        "label": f"Complete {n1} approved NQF level {level1} course(s)",
                        "required": n1 * 18,
                        "allow_unlisted_transcript_courses": True,
                        "transcript_filters": {"nqf_levels": [int(level1)]},
                        "approval_note": "Individual elective approval is not visible on the transcript.",
                        "verification_status": "unverified",
                    },
                ],
                f"{code.upper()} plus approved electives",
                f"{route_code}_{block_name}_mixed_{index}_branch1",
            )
            branch2 = {
                "type": "approved_credit_pool",
                "id": f"{route_code}_{block_name}_mixed_{index}_b",
                "label": f"Complete {n2} approved NQF level {level2} course(s)",
                "required": n2 * 18,
                "allow_unlisted_transcript_courses": True,
                "transcript_filters": {"nqf_levels": [int(level2)]},
                "approval_note": "Individual elective approval is not visible on the transcript.",
                "verification_status": "unverified",
            }
            rules.append(
                any_of(
                    [branch1, branch2],
                    f"{block_name}: complete one published elective alternative",
                    f"{route_code}_{block_name}_mixed_{index}",
                )
            )
            represented_lines.add(index)
        elif mixed_or:
                ...
                any_of(
                    [
                        course_rule(
                            [code.upper()],
                            f"Complete {code.upper()}",
                            f"{route_code}_{block_name}_mixed_or_{index}_course",
                        ),
                        {
                            "type": "approved_credit_pool",
                            "id": f"{route_code}_{block_name}_mixed_or_{index}_pool",
                            "label": f"Complete {n} approved NQF level {level} course(s)",
                            "required": n * 18,
                            "allow_unlisted_transcript_courses": True,
                            "transcript_filters": {"nqf_levels": [int(level)]},
                            "approval_note": "Individual elective approval is not visible on the transcript.",
                            "verification_status": "unverified",
                        },
                    ],
                    f"{block_name}: complete one published elective alternative",
                    f"{route_code}_{block_name}_mixed_or_{index}",
                )
            )
            represented_lines.add(index)
    ...
    for index, clean in enumerate(clean_lines):
        ...
        if "from:" in clean.lower() or "from the list" in clean.lower():
            continue
        if re.match(
            r"^(Total credits|Notes?|Additional information|Assessment|Readmission|Distinction|Graduation)",
            clean,
            re.I,
        ):
            continue
        if re.fullmatch(r"Elective Courses?:?\s*\.*", clean, re.I):
            continue
        if not re.search(
            r"elective|approved course|other NQF|other ECO|NQF level \d .*course|NQF \d .*course|2000 level course",
            clean,
            re.I,
        ):
            continue
        if re.search(
            r"suggested|recommended|not available|option to choose|may choose|may be taken|register for this elective|course consists",
            clean,
            re.I,
        ):
            continue
        ...
        if not (
            ".." in clean or re.match(r"^(Any|One|Two|Three|Four|Plus)", clean, re.I)
        ):
            continue
        context = clean
        if index + 1 < len(clean_lines) and re.search(
            r"credits|minimum", clean_lines[index + 1], re.I
        ):
        ...
        total_match = re.search(
            r"totalling\s+(?:a\s+)?(?:minimum\s+of\s+)?(\d+)\s+credits", context, re.I
        )
        credit_match = re.search(r"(?:\.{2,}|\s)(\d+)\+?\s+([5-8])\s*$", clean)
        level_match = re.search(
            r"NQF(?: level)?\s*([5-8])|([123])000 level|first year level", clean, re.I
        )
        nqf_level = (
            5
            if re.search(r"first year level", clean, re.I)
            else (int(next(v for v in level_match.groups() if v)) if level_match else 0)
        )
        if total_match:
            ...
            required_credits = (
                printed_credits * required_count
                if required_count > 1 and printed_credits <= 24
                else printed_credits
            )
        else:
            required_credits = required_count * 18
        prefixes: list[str] = []
        if re.search(r"\bECO\b", clean):
            prefixes = ["ECO"]
        if re.search(r"\bPOL\b", clean):
            prefixes = ["POL"]
        filters: dict[str, Any] = {}
        if nqf_level:
            filters["nqf_levels"] = [nqf_level]
        if prefixes:
        ...
        rules.append(
            {
                "type": "approved_credit_pool",
                "id": f"{route_code}_{block_name}_open_{index}",
                "label": f"{block_name}: {clean.strip(' .')}",
    ...
    unresolved = [
        ...
        for index, clean in enumerate(clean_lines)
        if re.search(
            r"\bOR\b|Plus|Elective Courses|approved courses|^Any ", clean, re.I
        )
    ]
    ...
    for line_index, line in unresolved:
        if line_index in represented_lines:
            continue
        if re.search(
            r"not offered|option to choose|or equivalent|supplementary", line, re.I
        ):
            continue
        codes_in_line = re.findall(r"\b[A-Z]{3}\d{4}[A-Z](?:/[A-Z])?\b", line)
        if codes_in_line and all(
            any(code in represented_text for code in expand_code(raw))
            for raw in codes_in_line
        ):
            continue
...
def advanced_curriculum(
    spec: dict[str, Any], segment: str, start_page: int
) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[str]]:
    ...
    if code == "CU020BUS01":
        ...
        prescribed = [
            row for row in rows if any(c in prescribed_codes for c in row["codes"])
        ]
        electives = [
            row for row in rows if not any(c in prescribed_codes for c in row["codes"])
        ]
        for row in prescribed:
            rules.append(row_rule(row, code, row["page"] * 100 + len(rules)))
        rules.append(
            choose(
                [c for row in electives for c in row["codes"]],
                2,
                "Complete two published elective courses",
                f"{code}_electives",
            )
        )
        warnings.append(
            "The distinction paragraph refers to five prescribed courses while the curriculum table lists six; the award calculation uses the best four of the six listed courses and is flagged for confirmation."
        )
...
def add_table_fact(courses: dict[str, dict[str, Any]], row: dict[str, Any]) -> None:
    for code in row["codes"]:
        ...
        courses[code] = {
            ...
            "nqf_level": row["level"],
            "prerequisites": [],
            "prerequisites_verified": row["level"] == 5,
            "co_requisites": [],
            ...
            "description": "",
            "verification_status": "verified",
            "source": source(row["page"], "Programme curriculum table"),
            "general_elective": False,
            "counts_towards_general_degree": True,
            "counts_as_humanities": False,
            "counts_as_science": False,
            "counts_towards_course_equivalents": True,
            "credit_bearing": row["credits"] > 0,
...
def programme_progression(
    spec: dict[str, Any], curriculum_codes: set[str]
) -> list[dict[str, Any]]:
            ...
            return [
                {
                    ...
                    "label": "Pass at least two courses in the first year",
                    "minimum": 2,
                },
                {
                    "type": "repeat_failure",
                    "label": "No course may be failed more than once",
                    "maximum_failures": 1,
                },
                {
                    "type": "maximum_years",
                    "label": "Complete within two years",
                    "maximum": 2,
                },
            ]
        if code == "CU021GSB48":
            return [
                {
                    ...
                    "label": "No more than three compulsory courses may be failed",
                    "threshold": 4,
                },
                {
                    "type": "repeat_failure",
                    "label": "A failed compulsory course may be repeated only once",
                    "maximum_failures": 1,
                },
            ]
        return [
            {
                "type": "repeat_failure",
                "label": "No programme course may be failed twice",
                "maximum_failures": 1,
            }
        ]
    ...
    rules.append(
        {
            "type": "repeat_failure",
            "label": "No required course may be failed more than once",
            "maximum_failures": 1,
        }
    )
    rules.append(
        {
            ...
            "label": "Annual failure load",
            "threshold": 5,
        }
    )
    if family == "bbussci":
        if mode == "extended":
            thresholds = {1: 3, 2: 7, 3: 13, 4: 19}
            annual = 3
        else:
            thresholds = {1: 4, 2: 10, 3: 18}
            annual = 4
        rules.append(
            {
                "type": "cumulative_failed_course_equivalents",
                "label": "Fewer than seven failed semester-course equivalents over the degree",
                "threshold": 7,
            }
        )
    else:
        if mode == "extended":
            thresholds = {1: 3, 2: 6, 3: 10, 4: 15}
            annual = 3
        else:
            thresholds = {1: 4, 2: 8, 3: 15}
            annual = 4
    for year, minimum in thresholds.items():
        rules.append(
            {
                "type": "course_equivalents_cumulative",
                "label": f"End of year {year} course-equivalent threshold",
                "year": year,
                "minimum": minimum,
            }
        )
    rules.append(
        {
            "type": "annual_course_equivalents",
            "label": "Annual completed course-equivalent threshold",
            "minimum": annual,
        }
    )
    rules.append(
        {
            "type": "maximum_years",
            "label": "Complete within the standard period unless Senate grants a concession",
            "maximum": spec["minimum_years"],
        }
    )
    ...
    if code.endswith(("BUS01", "BUS09")):
        ...
        rules.append(
            {
                "type": "failed_any",
                "label": "Actuarial first-year exclusion courses",
                "course_codes": actuarial_first,
            }
        )
    return rules


def degree_award_rules(
    spec: dict[str, Any], all_codes: list[str]
) -> list[dict[str, Any]]:
            ...
            return [
                {
                    ...
                    "curriculum_rules": [
                        {
                            "type": "best_n_average",
                            "id": "adv_actuarial_best4",
                            "label": "Best four prescribed results",
                            "course_codes": prescribed,
                            "required": 4,
                            "minimum_average": 75,
                    ...
                    "curriculum_rules": [
                        {
                            ...
                            "id": "adv_management_average",
                            "label": "Coursework weighted average",
                            "minimum_average": 75,
                        }
                    ],
                }
        ...
        return [
            {
                ...
                "curriculum_rules": [
                    {
                        "type": "weighted_average",
                        "id": "adv_accounting_average",
                        "label": "Four professional-subject average",
                        "course_codes": [
                            "ACC3009W",
                            "ACC3023W",
                            "ACC3022W",
                    ...
                    *[
                        {
                            "type": "minimum_mark",
                            "id": f"adv_acc_min_{c.lower()}",
                            "label": f"{c} minimum",
                            "course_codes": [c],
                            "minimum_mark": 60,
                        }
                        for c in ["ACC3009W", "ACC3023W", "ACC3022W", "ACC3004W"]
                    ],
                    {
                        "type": "no_failures",
                        "id": "adv_acc_first_attempt",
                        "label": "Only first-attempt passes count",
                        "course_codes": [
                            "ACC3009W",
                            "ACC3023W",
                            "ACC3022W",
    ...
    if suffix == "FTX05":
        if family == "bbussci":
            rules.append(
                {
                    "name": "Finance subject distinction",
                    "curriculum_rules": [
                        {
                            "type": "weighted_average",
                            "id": "finance3",
                            "label": "FTX3044F and FTX3045S average",
                            "course_codes": ["FTX3044F", "FTX3045S"],
                            "minimum_average": 75,
                        },
                        {
                            "type": "weighted_average",
                            "id": "finance4",
                            "label": "Final-year Finance average",
                            "course_codes": ["FTX4056S", "FTX4057F", "FTX4087S"],
                            "minimum_average": 75,
                        },
                        *[
                            {
                                "type": "minimum_mark",
                                "id": f"finance_min_{c.lower()}",
                                "label": f"{c} minimum",
                                "course_codes": [c],
                                "minimum_mark": 70,
                            }
                    ...
                    "curriculum_rules": [
                        {
                            "type": "weighted_average",
                            "id": "bcom_finance2",
                            "label": "Financial Management average",
                            "course_codes": ["FTX2024F", "FTX2024S"],
                            "minimum_average": 75,
                        },
                        {
                            "type": "weighted_average",
                            "id": "bcom_finance3",
                            "label": "Investment Management average",
                            "course_codes": ["FTX3044F", "FTX3045S"],
                            "minimum_average": 75,
                        },
                    ],
        ...
        rules.append(
            {
                "name": "Economics subject distinction",
                "curriculum_rules": [
                    {
                        "type": "best_n_average",
                        "id": "economics_best3",
                        "label": "ECO3020F plus two other third-year Economics courses",
                        "course_codes": eco3000,
                        "required": 3,
                        "mandatory_course_codes": ["ECO3020F"],
                        "minimum_average": 80,
                        "minimum_mark_count": 2,
                        "minimum_mark": 75,
                    },
                ],
            }
        ...
        final_inf = [
            c for c in all_codes if c.startswith("INF3") or c.startswith("INF4")
        ]
        rules.append(
            {
                "name": "Information Systems subject distinction",
                "curriculum_rules": [
                    {
                        "type": "weighted_average",
                        "id": "is_final_average",
                        "label": "Final-year Information Systems weighted average",
                        "course_codes": final_inf,
                        "minimum_average": 75,
                    }
                ],
            }
        ...
        rules.append(
            {
                "name": "Marketing subject distinction",
                "curriculum_rules": [
                    {
                        "type": "weighted_average",
                        "id": "marketing_average",
                        "label": "Marketing course weighted average",
                        "course_codes": marketing,
                        "minimum_average": 75,
                    },
                    *[
                        {
                            "type": "minimum_mark",
                            "id": f"marketing_min_{c.lower()}",
                            "label": f"{c} minimum",
                            "course_codes": [c],
                            "minimum_mark": 70,
                        }
        ...
        rules.append(
            {
                "name": "Industrial and Organisational Psychology subject distinction",
                "curriculum_rules": [
                    {
                        "type": "weighted_average",
                        "id": "iop_average",
                        "label": "BUS4006W and BUS4030H average",
                        "course_codes": ["BUS4006W", "BUS4030H"],
                        "minimum_average": 75,
                    },
                    {
                        "type": "minimum_mark",
                        "id": "iop_course1",
                        "label": "BUS4006W minimum",
                        "course_codes": ["BUS4006W"],
                        "minimum_mark": 70,
                    },
                    {
                        "type": "minimum_mark",
                        "id": "iop_course2",
                        "label": "BUS4030H minimum",
                        "course_codes": ["BUS4030H"],
                        "minimum_mark": 70,
                    },
                    manual(
                        "Coursework and research-report component subminima",
                        "iop_components",
                        "The transcript does not expose component marks; both coursework and the research report require at least 70%.",
                        blocking=True,
                    ),
                ],
            }
        ...
        rules.append(
            {
                "name": "Statistics subject distinction",
                "curriculum_rules": [
                    {
                        "type": "passed_mark_count",
                        "id": "stats2_firsts",
                        "label": "Two second-year Statistics firsts",
                        "course_codes": stats2,
                        "required": 2,
                        "minimum_mark": 75,
                    },
                    {
                        "type": "passed_mark_count",
                        "id": "stats3_firsts",
                        "label": "Two third-year Statistics firsts",
                        "course_codes": stats3,
                        "required": 2,
                        "minimum_mark": 75,
                    },
...
def programme_specific_rules(spec: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if code.endswith("CSC05"):
        for course_code in [
            ...
            rules.append(
                {
                    "type": "minimum_mark",
                    "id": f"cs65_{course_code.lower()}",
                    "label": f"Minimum 65% in {course_code}",
    ...
    if code.endswith("FTX05"):
        rules.append(
            {
                "type": "weighted_average",
                "id": "finance_entry_average",
                "label": "FTX3044F and FTX3045S progression average",
                "course_codes": ["FTX3044F", "FTX3045S"],
                "minimum_average": 60,
            }
        )
            ...
            [
                {
                    "type": "minimum_mark",
                    "id": "acc1011_60",
                    "label": "ACC1011S progression mark",
                    "course_codes": ["ACC1011S", "ACC1111S"],
                    "minimum_mark": 60,
                },
                {
                    "type": "weighted_average",
                    "id": "finance_accounting_entry",
                    "label": "FTX3044F and FTX3045S progression average",
                    "course_codes": ["FTX3044F", "FTX3045S"],
                    "minimum_average": 60,
                },
                manual(
                    "Final-year Chartered Accounting course admission",
                    "ca_course_admission",
                    "Admission to ACC3009W is subject to the published weighted-average, timing and subminimum rules and may depend on entrance examinations.",
    ...
    if code.endswith("ECO03"):
        rules.append(
            manual(
                "Allocation of limited Law-course places",
                "law_place_allocation",
                "The 63% eligibility threshold does not guarantee a place; allocation is competitive and subject to Law Faculty capacity and redress targets.",
                status="discretionary",
                blocking=True,
            )
        )
    if code.endswith(("BUS01", "BUS09")) and spec["family"] != "advanced":
        rules.append(
            manual(
                "Actuarial professional exemptions",
                "actuarial_exemptions",
                "Professional-body exemptions depend on individual course performance and external examiner decisions; they are not part of degree completion.",
                status="discretionary",
                blocking=False,
            )
        )
...
def main() -> None:
    ...
    for position_index, (start, spec) in enumerate(located):
        ...
        if spec["family"] == "advanced":
            curriculum_rules, table_rows, unresolved = advanced_curriculum(
                spec, segment, start_page
            )
        else:
            for block_name, block_lines in split_year_blocks(segment, start_page):
                block_rules, rows, warnings = parse_block(
                    block_name, block_lines, spec["code"]
                )
                curriculum_rules.extend(block_rules)
        ...
        if family == "advanced":
            ...
            minimum_credits, level7, duration, max_years = (
                528,
                0,
                spec["minimum_years"],
                spec["minimum_years"],
        ...
        curriculum_rules.insert(
            0,
            {
                "type": "credits",
                "id": f"{spec['code']}_total_credits",
                "label": "Minimum qualification credits",
                "required": minimum_credits,
            },
        )
        if family == "bbussci":
            curriculum_rules.insert(
                ...
                {
                    "type": "level_credits",
                    "id": f"{spec['code']}_level8",
                    "label": "Minimum NQF level 8 credits",
                    "nqf_level": 8,
                    "required": 120,
                },
            )
            ...
            curriculum_rules.insert(
                ...
                {
                    "type": "level_credits",
                    "id": f"{spec['code']}_level7",
                    "label": "Minimum NQF level 7 credits",
                    "nqf_level": 7,
                    "required": 120,
                },
            )
        ...
        printed_total_match = re.search(
            r"Total credits for the degree\D+(\d+)", segment, re.I
        )
        if printed_total_match and int(printed_total_match.group(1)) < minimum_credits:
            curriculum_rules.append(
                manual(
                    "Published programme-total reconciliation",
                    f"{spec['code']}_credit_conflict",
        ...
        if unresolved:
            if spec["code"] == "CU020BUS01" and all(
                "distinction paragraph" in item for item in unresolved
            ):
                curriculum_rules.append(
                    manual(
                        "Advanced Diploma distinction-source reconciliation",
                        f"{spec['code']}_distinction_conflict",
                        unresolved[0],
                        status="conflict",
                        blocking=False,
                    )
                ...
                curriculum_rules.append(
                    manual(
                        "Unresolved programme-table choice wording",
                        f"{spec['code']}_choice_verification",
        ...
        programmes[key] = {
            "name": spec["name"],
            "minimum_nqf_credits": minimum_credits,
            ...
            "required_courses": [],
            "minimum_duration_years": duration,
            ...
            "scope_verified": True,
            "route_type": spec["mode"],
            "degree_category": degree_category,
            "programme_type": "commerce_structured",
            "curriculum_rules": curriculum_rules,
            "pathways": {},
            ...
            "award_notes": [
                "Distinction calculations use only programme-relevant first attempts and do not round up."
            ],
    ...
    referenced_substitutes = sorted(
        {c for groups in SUBSTITUTIONS.values() for group in groups for c in group}
    )
    for code in referenced_substitutes:
        ...
        courses[code] = {
            "code": code,
            "name": f"Recognised substitute {code}",
            "credits": 18,
            "nqf_level": level,
            "prerequisites": [],
            ...
            "co_requisites": [],
            "offered": offered_from_code(code),
            "offering_verified": False,
            "department": PREFIX_DEPARTMENT.get(prefix(code), prefix(code)),
            "description": "",
            "verification_status": "provisional",
            "source": source(260, "Commerce interfaculty course substitutions"),
            "general_elective": False,
            "counts_towards_general_degree": True,
            "counts_as_humanities": False,
            "counts_as_science": False,
    ...
    (OUT / "courses.json").write_text(
        json.dumps([courses[c] for c in sorted(courses)], indent=2, ensure_ascii=False)
        + "\n"
    )
    (OUT / "degree_requirements.json").write_text(
        json.dumps(requirements, indent=2, ensure_ascii=False) + "\n"
    )
    source_dir = OUT / "source_extraction"
    source_dir.mkdir(exist_ok=True)
    (source_dir / "programme_extraction_summary.json").write_text(
    ...
    for course_code, observations in sorted(table_observations.items()):
        published = sorted(
            {(item["credits"], item["nqf_level"]) for item in observations}
        )
        if len(published) <= 1:
            continue
        ...
        conflicts.append(
            {
                ...
                "published_credit_level_pairs": [
                    {"credits": credits, "nqf_level": level}
                    for credits, level in published
                ],
                "canonical_course_fact": {
                    "credits": canonical.get("credits"),
                    "nqf_level": canonical.get("nqf_level"),
    ...
    (source_dir / "course_fact_conflicts.json").write_text(
        json.dumps(conflicts, indent=2, ensure_ascii=False) + "\n"
    )
    print(
        f"Wrote {len(courses)} Commerce course facts and {len(programmes)} programme routes; recorded {len(conflicts)} table conflicts"
    )
