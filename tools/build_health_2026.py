...
def extract_courses(
    ...
    for index, match in enumerate(matches):
        ...
        credit = re.search(
            r"(\d+)\s+NQF credits? at NQF level\s+(\d+)", segment[:3000], re.I
        )
        if not credit:
            continue
        ...
        not_offered = bool(
            re.search(
                r"not offered in 2026|not on offer in 2026",
                title + " " + segment[:800],
                re.I,
            )
        )
        ...
        description = (
            re.sub(r"\s+", " ", outline.group(1)).strip()[:1000] if outline else ""
        )
        ...
        if re.search(r"equivalent courses", equivalence_window, re.I):
            equivalent_codes.update(
                re.findall(r"\b[A-Z]{3}\d{4}[A-Z]?\b", equivalence_window)
            )
...
def programme(
    *,
    ...
    page: int,
    route_type: str = "structured",
    availability: str = "open",
    availability_note: str = "",
    admission_notes: list[str] | None = None,
    progression_notes: list[str] | None = None,
    ...
    return {
        ...
        "minimum_duration_years": minimum_years,
        "maximum_registration_years": maximum_years,
        ...
        "degree_category": "Health Sciences",
        "programme_type": "health_professional",
...
def main() -> None:
    ...
    facts: dict[str, tuple[str, int, int]] = {
        ...
        "AHS3072S": ("Paediatric Motor Speech Disorders and Dysphagia", 22, 7),
        "AHS3073F": ("Adult Motor Speech Disorders and Dysphagia", 22, 7),
        "AHS3102S": ("Adult Language Disorders", 15, 7),
        "AHS3103F": ("Fluency, Voice and Craniofacial Disorders", 15, 7),
        "AHS4000W": ("Research Project", 30, 8),
        "AHS4067S": ("Professional Practice", 4, 8),
        ...
        "HUB1019F": (
            "Anatomy and Physiology I for Health and Rehabilitation Sciences",
            18,
            5,
        ),
        "HUB1020S": (
            "Anatomy and Physiology II for Health and Rehabilitation Sciences",
            18,
            5,
        ),
        "AHS1032S": ("Introduction to Occupational Therapy", 20, 5),
        "AHS1035F": ("Occupation and Health", 22, 5),
        "AHS2002W": ("Professional Practice and Ethics", 13, 6),
        "PRY2002W": ("Mental Health", 14, 6),
        "PSY2013F": ("Social and Developmental Psychology", 20, 6),
        "HUB2015W": ("Anatomy and Physiology II for HRS", 36, 6),
        "AHS2043W": ("Occupational Therapy Theory and Practice", 36, 6),
        "AHS3107W": ("OT Theory and Practice in Physical Health", 38, 7),
        "AHS3108W": ("OT Theory and Practice in Mental Health", 38, 7),
        "AHS3113W": ("Foundation Theory for OT Practice I", 26, 7),
        ...
        "AHS4122X": (
            "Occupational Therapy Practice and Service Learning Part A",
            32,
            8,
        ),
        "AHS4123X": (
            "Occupational Therapy Practice and Service Learning Part B",
            16,
            8,
        ),
        "HUB1022F": ("Human Biology for Physiotherapy I", 9, 5),
        "AHS1033F": ("Movement Science I", 18, 5),
        "HUB1023S": ("Human Biology for Physiotherapy II", 9, 5),
        "AHS1034S": ("Introduction to Applied Physiotherapy", 22, 5),
        "HUB2023W": ("Human Biology for Physiotherapy III", 9, 6),
        "AHS2050H": ("Clinical Practice I", 18, 6),
        "AHS2052H": ("Movement Science II", 38, 6),
        "AHS2053H": ("Applied Physiotherapy II", 32, 6),
        "AHS3069W": ("Clinical Physiotherapy", 62, 7),
        "AHS3070H": ("Becoming a Rehabilitation Professional", 22, 7),
        "AHS3076H": ("Movement Science III", 24, 7),
        "AHS3077H": ("Applied Physiotherapy III", 22, 7),
        "AHS4065W": ("Clinical Physiotherapy", 98, 8),
        "AHS4066F": ("Becoming a Rehabilitation Professional", 4, 8),
        "AHS4185S": ("Becoming a Rehabilitation Professional", 4, 8),
        "AHS4071F": ("Applied Physiotherapy", 20, 8),
        "AHS4184S": ("Applied Physiotherapy", 20, 8),
        "AHS4072H": ("Research Methods and Biostatistics II", 10, 8),
        "MDN3005W": ("Cosmetic Science and Skin Biology", 30, 7),
        "MDN3006W": ("Cosmetic Formulation", 30, 7),
        "MDN3007W": ("Cosmetic Product Development", 30, 7),
        "MDN3008W": ("Cosmetic Regulation and Safety", 15, 7),
        "MDN3009W": ("Cosmetic Business and Marketing", 15, 7),
        "MDN3010W": ("In-service Training", 0, 7),
        "CEM1011F": ("Chemistry for Medical Students", 18, 5),
        "PHY1025F": ("Physics for Medical Students", 18, 5),
        "SLL1044S": ("Language for Medical Students I", 18, 5),
        "SLL1041S": ("Language for Medical Students II", 18, 5),
        ...
        "FCE2000W": ("Becoming a Doctor Part IA", 21, 6),
        "SLL2002H": ("Becoming a Doctor Part IB", 18, 6),
        "HSE2000W": ("Becoming a Doctor Part IC", 22, 6),
        "HUB2017H": ("Integrated Health Systems Part IA", 57, 6),
        "FCE3000F": ("Becoming a Doctor Part IIA", 10, 7),
        "MDN3001S": ("Becoming a Doctor Part IID", 68, 7),
        "SLL3002F": ("Becoming a Doctor Part IIB", 30, 7),
        "HSE3000F": ("Becoming a Doctor Part IIC", 15, 7),
        "PTY3009F": ("Integrated Health Systems Part II", 59, 7),
        "MDN2001S": ("Special Study Module", 16, 6),
        "SLL3003W": ("Language and Clinical Practice", 0, 8),
        "PRY4000W": ("Psychiatry", 30, 8),
        "AAE4002W": ("Anaesthesia Part I", 20, 8),
        "OBS4003W": ("Obstetrics", 30, 8),
        "MDN4011W": ("Medicine", 40, 8),
        "MDN4001W": ("Clinical Methods", 20, 8),
        "MDN4015W": ("Integrated Clinical Practice", 20, 8),
        "PED4016W": ("Paediatrics", 10, 8),
        "PED4049W": ("Child Health", 10, 8),
        "PPH4056W": ("Health in Context", 40, 8),
        "PED5005W": ("Paediatrics I", 10, 8),
        "PED5006W": ("Paediatrics II", 30, 8),
        "CHM5003W": ("Surgery I", 40, 8),
        "MDN5003H": ("Medicine I", 20, 8),
        "CHM5004H": ("Surgery II", 10, 8),
        "OBS5005W": ("Obstetrics and Gynaecology", 20, 8),
        "CHM5005H": ("Surgical Specialties I", 10, 8),
        "MDN5005W": ("Medicine II", 10, 8),
        "MDN5006W": ("Medicine III", 10, 8),
        "CHM5007W": ("Surgical Specialties II", 20, 8),
        "CHM5008W": ("Surgical Specialties III", 10, 8),
        "CHM5009W": ("Surgical Specialties IV", 10, 8),
        "CHM5010W": ("Surgical Specialties V", 10, 8),
        "CHM6000W": ("Surgery", 41, 8),
        "MDN6000W": ("Medicine", 41, 8),
        "OBS6000W": ("Obstetrics and Gynaecology", 41, 8),
        "PED6000W": ("Paediatrics and Child Health", 30, 8),
        "PED6004W": ("Child Health", 10, 8),
        "FCE6000W": ("Family Medicine and Palliative Care", 21, 8),
        "PRY6000W": ("Psychiatry", 21, 8),
        "AAE6000W": ("Anaesthesia", 10, 8),
        "FCE6001W": ("Primary Care", 19, 8),
        "FCE6005W": ("Emergency Medicine", 10, 8),
        "PTY6012W": ("Forensic Pathology", 10, 8),
        "HSE6004W": ("Final Integrated Assessment", 0, 8),
        "HUB3006F": ("Applied Human Biology", 36, 7),
        "HUB3007S": ("Human Neurosciences", 36, 7),
        "IBS3020W": ("Molecular Medicine", 72, 7),
        "AAE4003W": ("Anaesthesia NMFC", 8, 8),
        "MDN4017W": ("Medicine NMFC", 15, 8),
        "PED4017W": ("Paediatrics NMFC", 7, 8),
        "OBS4006W": ("Obstetrics NMFC", 15, 8),
        "PRY4001W": ("Psychiatry NMFC", 15, 8),
        "AAE5000W": ("Anaesthesia NMFC", 10, 8),
        "PTY5012W": ("Forensic Pathology NMFC", 10, 8),
        "OBS5006W": ("Obstetrics NMFC", 27, 8),
        "MDN5000W": ("Medicine NMFC", 24, 8),
        "OBS5007W": ("Gynaecology NMFC", 41, 8),
        "PED5004W": ("Paediatrics NMFC", 40, 8),
        "PRY5001W": ("Psychiatry NMFC", 30, 8),
        "CHM5011W": ("Surgery NMFC", 19, 8),
        "AHS1060F": ("Disability Practice Module 1", 7, 5),
        "AHS1060S": ("Disability Practice Module 1", 7, 5),
        "AHS1061F": ("Disability Practice Module 2", 8, 5),
        "AHS1061S": ("Disability Practice Module 2", 8, 5),
        "AHS1062F": ("Disability Practice Module 3", 10, 5),
        "AHS1062S": ("Disability Practice Module 3", 10, 5),
        "AHS1063F": ("Disability Practice Module 4", 15, 5),
        "AHS1063S": ("Disability Practice Module 4", 15, 5),
        "AHS1064F": ("Disability Practice Module 5", 15, 5),
        "AHS1064S": ("Disability Practice Module 5", 15, 5),
        "AHS1065F": ("Disability Practice Module 6", 15, 5),
        "AHS1065S": ("Disability Practice Module 6", 15, 5),
        "AHS1066F": ("Disability Practice Module 7", 25, 5),
        "AHS1066S": ("Disability Practice Module 7", 25, 5),
        "AHS1067F": ("Disability Practice Module 8", 25, 5),
        "AHS1067S": ("Disability Practice Module 8", 25, 5),
    ...
    for code, (name, credits, level) in facts.items():
        if code not in courses:
            courses[code] = {
                ...
                "nqf_level": level,
                "prerequisites": [],
                "prerequisites_verified": level == 5,
                "co_requisites": [],
                ...
                "description": "",
                "verification_status": "verified",
                "source": source(42, "Published programme curriculum table"),
                "general_elective": False,
                "counts_towards_general_degree": True,
                "counts_as_humanities": False,
                "counts_as_science": False,
    ...
    def direct_rules(codes: Iterable[str], stem: str) -> list[dict[str, Any]]:
        return [
            course_rule(accepted(code), courses[code]["name"], f"{stem}_{code.lower()}")
            for code in codes
        ]
    ...
    hpcsa_manual = manual(
        "HPCSA and Faculty professional requirements",
        "hpcsa_confirmation",
    ...
    hrs_progress = [
        ...
        },
        {
            ...
            "label": "From second year, do not fail half or more of the annual courses",
            "threshold": 0.5,
        },
        {
    ...
    comm_common_rules = direct_rules(comm_common, "comm") + [
        course_rule(
            accepted("PSY1004F"), "Introduction to Psychology Part 1", "comm_psy1"
        ),
        course_rule(
            accepted("PSY1005S"), "Introduction to Psychology Part 2", "comm_psy2"
        ),
        course_rule(
            ["SLL1028H", "SLL1048H"],
            "Approved Health and Rehabilitation Sciences language course",
            "comm_language",
        ),
        clinical_manual,
        hpcsa_manual,
    ...
    comm_progress_base = hrs_progress + [
        {
            ...
            "label": "First-year semester failure threshold",
            "groups": [
                {
                    "label": "First semester communication-sciences courses",
                    "course_codes": first_sem_common,
                    "threshold": 2,
                },
            ],
        },
        {
            "type": "maximum_years",
            "label": "Ordinary maximum registration period",
            "maximum": 5,
        },
    ]
    ...
    ot_rules = direct_rules(ot_codes, "ot") + [
        course_rule(
            accepted("PSY1004F"), "Introduction to Psychology Part 1", "ot_psy1"
        ),
        course_rule(
            accepted("PSY1005S"), "Introduction to Psychology Part 2", "ot_psy2"
        ),
        any_of(
            [
                ...
                all_courses(
                    ["AHS4122X", "AHS4123X"],
                    "Complete the adjusted service-learning pair",
                    "ot_service_adjusted",
                ),
            ],
            "Occupational Therapy practice and service learning",
            "ot_service",
        ),
        clinical_manual,
        hpcsa_manual,
    ...
    phys_rules = direct_rules(phys_codes, "phys") + [
        course_rule(
            accepted("PSY1004F"), "Introduction to Psychology Part 1", "phys_psy1"
        ),
        course_rule(
            ["SLL1028H", "SLL1048H"],
            "Approved Health and Rehabilitation Sciences language course",
        ...
        course_rule(
            ["AHS4066F", "AHS4185S"],
            "Becoming a Rehabilitation Professional",
            "phys_professional",
        ),
        ...
        for code in codes:
            mbchb_rules.append(
                course_rule(
                    accepted(code),
                    courses[code]["name"],
                    f"mbchb_y{year}_{code.lower()}",
                )
            )
    mbchb_rules += [
        course_rule(ssm, "Complete one approved Special Study Module", "mbchb_ssm"),
        clinical_manual,
        hpcsa_manual,
    ]
    mbchb_progress = [
        ...
        {
            "type": "failed_course_fraction",
            "label": "Do not fail more than half of the annual courses",
            "threshold": 0.5,
        },
        {
            "type": "repeat_year_failure",
    ...
    bscmed_rules = [
        {
            "type": "credits",
            "id": "bscmed_total",
            "label": "At least 360 approved credits",
            "required": 360,
        },
        {
            "type": "level_credits",
            "id": "bscmed_level7",
            "label": "At least 120 level-7 credits",
            "nqf_level": 7,
            "required": 120,
        },
        {
            "type": "maximum_credit_pool",
            "id": "bscmed_level5_cap",
            "label": "No more than 96 level-5 credits",
            "course_codes": bscmed_recognised,
            "filters": {"nqf_levels": [5]},
            "maximum": 96,
        },
    ]
    ...
    for number in range(1060, 1068):
        split_groups.append(
            course_rule(
                [f"AHS{number}F", f"AHS{number}S"],
                f"Complete AHS{number} in either semester",
                f"disability_{number}",
            )
        )
    combined = all_of(
        [
            ...
            course_rule(
                ["AHS1062F", "AHS1062S"], "Module 3", "disability_1062_combined"
            ),
            course_rule("AHS1069W", "Combined modules 4 and 5", "disability_1069"),
            course_rule(
                ["AHS1065F", "AHS1065S"], "Module 6", "disability_1065_combined"
            ),
            course_rule("AHS1070W", "Combined modules 7 and 8", "disability_1070"),
        ],
        "Combined whole-year curriculum",
        "disability_combined",
    )
    disability_rules = [
        any_of(
            [
                all_of(split_groups, "Split-semester curriculum", "disability_split"),
                combined,
            ],
            "Complete an approved 120-credit Disability Practice curriculum",
            "disability_curriculum",
        ),
        clinical_manual,
    ]
    ...
    nmfc_rules = direct_rules(nmfc_codes, "nmfc") + [
        manual(
            "External qualification and final examination",
            "nmfc_external_award",
            "This collaboration route is controlled with the National Department of Health and the Cuban training institution; UCT does not independently confer the medical qualification from this transcript segment.",
            status="discretionary",
        )
    ]
    ...
    programmes["advanced_diploma_cosmetic_formulation"] = programme(
        ...
        minimum_years=1,
        maximum_years=2,
        required=cosmetic_codes,
        rules=[],
        progression=[
            {
                "type": "failed_course_count",
                "label": "Do not fail more than one course in the year",
                "threshold": 2,
            },
            {
                "type": "repeat_failure",
        ...
        awards=cosmetic_award,
        page=42,
        route_type="advanced_diploma",
        progression_notes=[
            "The in-service training course must be completed and recorded."
        ],
    )
    ...
    def add_comm(
        ...
        progression = comm_progress_base[:-1] + [
            {
                "type": "failed_courses_by_group",
                "label": "Second-semester first-year failure threshold",
                "groups": [
                    {
                        "label": "Second semester professional courses",
                        "course_codes": second_group,
                        "threshold": 2,
                    }
                ],
            },
        ...
        programmes[key] = programme(
            ...
            level7=120,
            level8=96,
            minimum_years=5 if fundamentals else 4,
            ...
            awards=hrs_award,
            page=43,
            route_type="fundamentals" if fundamentals else "structured",
            admission_notes=[
                "Professional-programme selection and capacity remain Faculty decisions."
            ],
            progression_notes=[
                "Clinical course progression can depend on subcomponents and placement performance not visible on an academic transcript."
            ],
            award_notes=["Distinction requires an overall programme average of 75%."],
        )

    add_comm("bsc_audiology", "BSc Audiology", "MB011", aud_codes, aud_second, False)
    add_comm(
        "bsc_audiology_fundamentals",
        "BSc Audiology — Fundamentals of Health Sciences route",
        "MB019",
        aud_codes,
        aud_second,
        True,
    ...
    add_comm(
        ...
        "BSc Speech-Language Pathology — Fundamentals route",
        "MB018",
        slp_codes,
        slp_second,
        True,
    ...
    def add_hrs(
        ...
        fundamentals: bool,
        page: int,
        first_year_groups: list[dict[str, Any]],
        *,
        level7: int = 120,
    ) -> None:
        ...
        progression = list(hrs_progress) + [
            {
                "type": "failed_courses_by_group",
                "label": "First-year semester failure threshold",
                "groups": first_year_groups,
            },
            {
                "type": "maximum_years",
                "label": "Ordinary maximum registration period",
        ...
        programmes[key] = programme(
            ...
            route_type="fundamentals" if fundamentals else "structured",
            admission_notes=[
                "Clinical placement and professional-programme admission remain Faculty decisions."
            ],
            progression_notes=[
                "All theoretical prerequisites and prior-year clinical requirements must be completed before progression where specified."
            ],
            award_notes=["Distinction requires an overall programme average of 75%."],
        )
    ...
    ot_groups = [
        {
            "label": "Occupational Therapy first semester",
            "course_codes": [
                "PSY1004F",
                "PSY1009F",
                "PSY1006F",
        ...
        {
            "label": "Occupational Therapy second semester",
            "course_codes": [
                "PSY1005S",
                "PSY1010S",
                "PSY1007S",
    ...
    phys_groups = [
        {
            "label": "Physiotherapy first semester",
            "course_codes": [
                "PPH1001F",
                "PSY1004F",
                "PSY1009F",
        ...
        {
            "label": "Physiotherapy second semester",
            "course_codes": ["PPH1002S", "HUB1020S", "HUB1023S", "AHS1034S"],
            "threshold": 2,
        },
    ]
    ...
    ot_rules_with_conflict = ot_rules + [
        manual(
            "Published Occupational Therapy credit reconciliation",
            "ot_credit_conflict",
            "The printed year totals and HEQSF level-7 minimum do not reconcile with the listed course rows. Faculty confirmation is required before a definitive graduation conclusion.",
            status="conflict",
        )
    ]
    add_hrs(
    ...
    add_hrs(
        ...
        "BSc Occupational Therapy — Fundamentals route",
        "MB016",
        ot_rules_with_conflict,
        True,
        56,
    ...
    add_hrs(
        ...
        "BSc Physiotherapy — Fundamentals route",
        "MB017",
        phys_rules,
        True,
        59,
    ...
    programmes["mbchb"] = programme(
        name="Bachelor of Medicine and Bachelor of Surgery",
        code="MB014",
        minimum_credits=1214,
        level7=120,
        level8=96,
        minimum_years=6,
        maximum_years=7,
        required=[],
        rules=mbchb_rules,
        progression=mbchb_progress,
        awards=[],
        page=48,
        route_type="clinical",
        pathways=mbchb_pathways,
        pathway_required=True,
        admission_notes=[
            "Admission and progression through clinical placements are controlled by the Faculty and professional requirements."
        ],
        progression_notes=[
            "Students ordinarily complete all prescribed courses of an academic year before entering the next year; years 4–6 use modular clinical blocks."
        ],
        award_notes=["Award calculations differ by first-registration cohort."],
    )
    mb_fund_rules = [
        course_rule(
            ["HSE1001F", "HSE1001S"],
    ...
    programmes["mbchb_fundamentals"] = programme(
        name="MBChB — Fundamentals of Health Sciences route",
        code="MB020",
        minimum_credits=1274,
        level7=120,
        level8=96,
        minimum_years=7,
        maximum_years=8,
        required=[],
        rules=mb_fund_rules,
        progression=mb_fund_progress,
        awards=[],
        page=62,
        route_type="fundamentals_clinical",
        pathways=mbchb_pathways,
        pathway_required=True,
        admission_notes=["Placement in the Fundamentals route is a Faculty decision."],
        progression_notes=[
            "The Fundamentals course must be passed before progression into the standard MBChB curriculum."
        ],
        award_notes=["Award calculations differ by first-registration cohort."],
    )

    programmes["bsc_medicine"] = programme(
        name="BSc Medicine",
        code="MB001",
        minimum_credits=360,
        level7=120,
        ...
        progression=[
            {
                "type": "maximum_years",
                "label": "Complete the BSc Medicine route within one year",
                "maximum": 1,
            }
        ],
        awards=[
        ...
        admission_notes=[
            "This route is available to eligible UCT MBChB students and admission is competitive."
        ],
        progression_notes=[
            "Only approved courses and credits can be recognised toward the qualification."
        ],
        award_notes=["Distinction requires a cumulative GPA of at least 75%."],
    )
    programmes["higher_certificate_disability_practice"] = programme(
        ...
        maximum_years=2,
        required=[],
        rules=disability_rules,
        progression=[
            {
                "type": "failed_course_count",
                "label": "First-year course failure threshold",
                "threshold": 2,
            },
            {
                "type": "repeat_failure",
        ...
        awards=[],
        page=64,
        route_type="higher_certificate",
        admission_notes=[
            "Practice-learning and admission requirements may require current programme confirmation."
        ],
    )
    programmes["nmfc_medical_training"] = programme(
        ...
        awards=[],
        page=65,
        route_type="restricted_collaboration",
        availability="restricted",
        availability_note="Restricted collaboration route for students designated under the NMFC programme.",
        admission_notes=[
            "National Department of Health and partner-institution rules govern admission and final qualification."
        ],
    )
    ...
    data = {
        "catalogue_version": "2026.1-health",
        "source": PDF_NAME,
    ...
    OUT.joinpath("courses.json").write_text(
        json.dumps(
            sorted(courses.values(), key=lambda row: row["code"]),
            indent=2,
            ensure_ascii=False,
        )
        + "\n"
    ...
    extraction.joinpath("course_descriptions.json").write_text(
        json.dumps(
            sorted(courses.values(), key=lambda row: row["code"]),
            indent=2,
            ensure_ascii=False,
    ...
    extraction.joinpath("equivalent_course_map.json").write_text(
        json.dumps(aliases, indent=2, ensure_ascii=False) + "\n"
    )
    print(
        f"Wrote {len(courses)} course facts and {len(programmes)} Health Sciences programme routes to {OUT}"
    )
