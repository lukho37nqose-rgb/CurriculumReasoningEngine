...
def _nqf_level_from_code(code: str) -> int:
    """Compatibility shim for legacy callers; UCT owns code-level inference."""
...
def load_catalogue(
    ...
    for raw in raw_courses:
        ...
        try:
            ...
            credits = 0
        try:
            explicit_level = raw.get("nqf_level", 0)
            level = int(explicit_level or code_scheme.infer_academic_level(code))
        except (TypeError, ValueError):
            level = 0
        ...
        prereq_verified = bool(
            raw.get("prerequisites_verified", prereq_valid and (bool(prereqs) or level == 5))
        )
        ...
        raw_prerequisite_expression = raw.get("prerequisite_expression")
        prerequisite_expression = None
        if raw_prerequisite_expression is not None:
            if isinstance(raw_prerequisite_expression, dict):
                prerequisite_expression = dict(raw_prerequisite_expression)
            else:
                prerequisite_expression = {"type": "__malformed__"}
                issues.append(f"{code} has a malformed canonical prerequisite expression.")
        ...
        courses[code] = CourseFact(
            ...
            general_elective=bool(
                raw.get("general_elective", raw.get("counts_towards_general_degree", True))
            ),
    ...
    for key, raw in raw_reqs.get("majors", {}).items():
        ...
        major = MajorDefinition(
            ...
            required_courses=[
                str(c).strip().upper() for c in raw.get("required_courses", []) if str(c).strip()
            ],
    ...
    for key, raw in raw_reqs.get("programmes", {}).items():
        ...
        for pathway_key, pathway_raw in raw.get("pathways", {}).items():
            ...
            pathways[pathway_key] = PathwayDefinition(
                ...
                required_courses=[
                    str(c).strip().upper() for c in pathway_raw.get("required_courses", []) if str(c).strip()
                ],
                ...
                progression_rules=[
                    r for r in pathway_raw.get("progression_rules", []) if isinstance(r, dict)
                ],
                award_rules=[r for r in pathway_raw.get("award_rules", []) if isinstance(r, dict)],
                source=(
                    pathway_raw.get("source", {}) if isinstance(pathway_raw.get("source", {}), dict) else {}
                ),
            )
        ...
        programmes[key] = ProgrammeRules(
            ...
            level_7_nqf_credits=int(
                raw.get("minimum_nqf_level_7_credits", raw.get("level_7_nqf_credits", 120))
            ),
            ...
            maximum_registration_years=(
                int(raw["maximum_registration_years"]) if raw.get("maximum_registration_years") else None
            ),
            ...
            elective_course_codes=[
                str(c).strip().upper() for c in raw.get("elective_course_codes", []) if str(c).strip()
            ],
            ...
            introductory_course_options=[
                str(c).strip().upper() for c in raw.get("introductory_course_options", [])
            ],
            ...
            support_course_codes=[
                str(c).strip().upper() for c in raw.get("support_course_codes", []) if str(c).strip()
            ],
            ...
            prerequisite_expression_overrides={
                str(code).strip().upper(): dict(expression)
                for code, expression in raw.get("prerequisite_expression_overrides", {}).items()
                if isinstance(expression, dict)
            },
            source=(raw.get("source", {}) if isinstance(raw.get("source", {}), dict) else {}),
            qualification_completion=(
                dict(raw.get("qualification_completion", {}))
                if isinstance(raw.get("qualification_completion", {}), dict)
                else {}
            ),
            graduation_eligibility=(
                dict(raw.get("graduation_eligibility", {}))
                if isinstance(raw.get("graduation_eligibility", {}), dict)
                else {}
            ),
            programme_entry_eligibility=raw.get("programme_entry_eligibility", {}),
